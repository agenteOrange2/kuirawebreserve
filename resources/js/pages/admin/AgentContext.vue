...
async function refreshPrompt() {
    ...
    try {
        const { data } = await axios.get<{ prompt: string }>(route('admin.ai.tenants.prompt', props.tenant.id));
        promptText.value = data.prompt;
    } catch (e: any) {
        toast.error('No se pudo actualizar', e.response?.data?.message ?? 'Ocurrió un error al cargar el prompt.');
    } finally {
...
async function toggleContextEditable() {
    ...
    try {
        await axios.patch(route('admin.ai.tenants.update', props.tenant.id), { context_editable: value });
...
async function toggleGuidelinesEditable() {
    ...
    try {
        await axios.patch(route('admin.ai.tenants.update', props.tenant.id), { guidelines_editable: value });
        toast.success('Visibilidad actualizada');
    } catch (e: any) {
        guidelinesEditableLocal.value = !value;
        toast.error('No se pudo actualizar', e.response?.data?.message ?? 'Ocurrió un error.');
        ...
        <div class="mt-2">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Instrucciones de plataforma y prompt efectivo de {{ tenant.name }}</p>
                </div>
            ...
            <div class="mt-5 grid grid-cols-12 gap-5">
                ...
                <div class="col-span-12 flex flex-col gap-5 xl:col-span-7">
                    <div class="box box--stacked flex flex-1 flex-col p-5">
                        <h2 class="text-base font-medium">Instrucciones de plataforma</h2>
                        <p class="mt-1 text-sm text-slate-500">
                            Definen cómo cotiza, cómo aparta y qué dice de pagos este bot; van por encima de las instrucciones del hotel y
                            debajo de las reglas de seguridad — nunca cobra ni confirma reservas.
                        </p>
                        <FormTextarea
                        ...
                        />
                        <FormHelp>Vacío = sin instrucciones extra de plataforma.</FormHelp>
                    ...
                    <div class="box box--stacked flex items-start justify-between gap-4 p-5">
                        <div class="min-w-0">
                            <div class="text-sm font-medium">El hotel puede ver y editar su contexto</div>
                            <FormHelp>
                                Habilita la página Contexto del bot en el panel del hotel (/asistente/contexto); apagado, solo la
                                plataforma gestiona este contexto.
                            </FormHelp>
                        </div>
                        <FormSwitch class="mt-1 shrink-0">
                            <FormSwitch.Input :checked="contextEditableLocal" type="checkbox" @change="toggleContextEditable" />
                        </FormSwitch>
                    </div>
                    ...
                    <div class="box box--stacked flex items-start justify-between gap-4 p-5">
                        <div class="min-w-0">
                            <div class="text-sm font-medium">El hotel puede capturar aprendizajes del bot</div>
                            <FormHelp>
                                Habilita la página Aprendizajes (/asistente/aprendizajes) y el botón "Enseñar al asistente" en su
                                Bandeja: correcciones de conversaciones reales que se inyectan al prompt. Apagado, las lecciones
                                las gestiona solo la plataforma.
                            </FormHelp>
                        </div>
                        <FormSwitch class="mt-1 shrink-0">
                            <FormSwitch.Input :checked="guidelinesEditableLocal" type="checkbox" @change="toggleGuidelinesEditable" />
                        </FormSwitch>
                    </div>
                </div>
                    ...
                    <div class="box box--stacked flex h-full flex-col p-5">
                        <div class="flex flex-wrap items-start justify-between gap-2">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-sm text-slate-500">
                                    Así ve el mundo este bot: identidad, datos del hotel, tarifas, FAQs, tus instrucciones y las reglas —
                                    armado en vivo.
                                </p>
                            </div>
                        ...
                        <pre class="mt-4 max-h-[70vh] flex-1 overflow-auto whitespace-pre-wrap break-words rounded bg-slate-50 p-4 font-mono text-xs text-slate-600 dark:bg-darkmode-700 dark:text-slate-300">{{ promptText }}</pre>
                    </div>
                </div>
            </div>
        </div>
