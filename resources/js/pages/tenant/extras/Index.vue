...
const form = reactive({ name: '', description: '', price: '' as string | number, sort_order: 0 as string | number, active: true });
...
async function submit() {
    ...
    try {
        if (editing.value) {
            const { data } = await axios.patch<ExtraRow>(`/api/extras/${editing.value.id}`, payload);
            extras.value = extras.value.map((e) => (e.id === data.id ? { ...data, price: Number(data.price) } : e));
            toast.success('Extra actualizado', 'Las reservas ya hechas conservan su precio congelado.');
        } else {
            const { data } = await axios.post<ExtraRow>('/api/extras', payload);
            extras.value = [...extras.value, { ...data, price: Number(data.price) }];
            toast.success('Extra agregado', 'El wizard ya puede ofrecerlo en el paso de extras.');
        }
...
async function toggleActive(extra: ExtraRow) {
    try {
        const { data } = await axios.patch<ExtraRow>(`/api/extras/${extra.id}`, { active: !extra.active });
        extras.value = extras.value.map((e) => (e.id === data.id ? { ...e, active: data.active } : e));
    } catch {
...
async function destroy() {
    ...
    try {
        ...
        toast.success('Extra eliminado', 'Las reservas existentes conservan sus líneas congeladas.');
        deleting.value = null;
    } catch {
    ...
    <RazeLayout title="Extras de reserva">
        <div class="mt-2">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-0.5 text-sm text-slate-500">
                        Add-ons que el huésped agrega a su reserva y suman al total: decoración, desayuno, late checkout.
                        El anticipo y el saldo los incluyen solos.
                    </p>
                </div>
            ...
            <div class="mt-5 box box--stacked">
                <div v-if="extras.length" class="overflow-auto p-5 lg:overflow-visible">
                    <Table>
                        <Table.Thead>
                            <Table.Tr>
                                <Table.Th class="whitespace-nowrap">Extra</Table.Th>
                                <Table.Th class="whitespace-nowrap">Precio</Table.Th>
                                <Table.Th class="whitespace-nowrap">Visible en el wizard</Table.Th>
                                ...
                                <Table.Td>
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-primary/10 bg-primary/10">
                                            <Lucide icon="Gift" class="h-4 w-4 text-primary" />
                                        </div>
                                        <div class="min-w-0">
                                            <div class="font-medium" :class="{ 'text-slate-400': !extra.active }">{{ extra.name }}</div>
                                            <div v-if="extra.description" class="mt-0.5 max-w-md truncate text-xs text-slate-500">{{ extra.description }}</div>
                                        </div>
                                    </div>
                                </Table.Td>
                                <Table.Td class="font-medium">{{ money(extra.price) }}</Table.Td>
                                <Table.Td>
                                    <FormSwitch v-if="canManage" title="Solo los extras activos se ofrecen al huésped">
                                        <FormSwitch.Input :checked="extra.active" type="checkbox" @change="toggleActive(extra)" />
                                    </FormSwitch>
                                    <span v-else class="text-xs" :class="extra.active ? 'text-success' : 'text-slate-400'">
                                        {{ extra.active ? 'Activo' : 'Pausado' }}
                                    </span>
                                </Table.Td>
                                <Table.Td>
                                    <div v-if="canManage" class="flex items-center justify-end gap-3">
                                        <a href="#" class="flex items-center text-primary" title="Editar" @click.prevent="openForm(extra)">
                                            <Lucide icon="Pencil" class="h-4 w-4" />
                                        </a>
                                        <a href="#" class="flex items-center text-danger" title="Eliminar" @click.prevent="deleting = extra">
                ...
                <div v-else class="flex flex-col items-center gap-3 px-5 py-12 text-center">
                    ...
                    <div>
                        <p class="text-sm font-medium text-slate-600">Aún no tienes extras</p>
                        <p class="mt-0.5 text-xs text-slate-500">
                            Crea tu primer add-on — decoración romántica, desayuno a la habitación, late checkout — y el wizard lo ofrecerá al reservar.
                        </p>
                    </div>
            ...
            <Dialog.Panel>
                <form class="p-5" @submit.prevent="submit">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-base font-medium">{{ editing ? `Editar ${editing.name}` : 'Nuevo extra' }}</h2>
                            <p class="text-xs text-slate-500">El precio se congela en cada reserva al momento de crearla.</p>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <div>
                            <label class="mb-1 block text-sm">Nombre</label>
                            <FormInput v-model="form.name" type="text" placeholder="Decoración romántica" />
                            <FormHelp v-if="errors.name" class="text-danger">{{ errors.name }}</FormHelp>
                        </div>
                        <div>
                            <label class="mb-1 block text-sm">Descripción (opcional)</label>
                            <FormTextarea v-model="form.description" rows="2" placeholder="Pétalos, globos y botella de espumoso a la llegada…" />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <FormInput v-model="form.price" type="number" step="0.01" min="0" placeholder="350.00" />
                                <FormHelp v-if="errors.price" class="text-danger">{{ errors.price }}</FormHelp>
                            </div>
                            <div>
                                <label class="mb-1 block text-sm">Orden</label>
                                <FormInput v-model.number="form.sort_order" type="number" min="0" />
                            </div>
                        </div>
                    ...
                    <div class="mt-5 flex justify-end gap-2">
                        ...
                        <Button type="submit" variant="primary" :disabled="saving">
                            {{ saving ? 'Guardando…' : editing ? 'Guardar cambios' : 'Agregar extra' }}
                        </Button>
                    </div>
                </form>
            </Dialog.Panel>
            ...
            <Dialog.Panel>
                <div class="p-5 text-center">
                    ...
                    <p class="mt-2 text-sm text-slate-500">
                        Las reservas que ya lo incluyen conservan su línea congelada. Si solo quieres dejar de ofrecerlo, usa el switch.
                    </p>
