...
const typesWithoutRate = computed(
    () =>
        new Set(
            props.roomTypes.filter((t) => !t.has_active_rate).map((t) => t.id),
        ),
);
...
const limitPercent = computed(() =>
    props.maxRooms
        ? Math.min(100, Math.round((props.rooms.length / props.maxRooms) * 100))
        : 0,
);
...
const filteredRooms = computed(() =>
    props.rooms.filter((r) => {
        ...
        if (
            ...
            return false;
        if (filters.zone !== '' && r.zone_id !== Number(filters.zone))
            return false;
...
const selectedFormType = computed(
    () =>
        props.roomTypes.find((t) => t.id === Number(form.room_type_id)) ?? null,
);
...
const occupancyPreview = computed(() => {
    ...
    const maxOccupancy =
        form.max_occupancy === '' ? typeCapacity : Number(form.max_occupancy);
    const included =
        form.included_occupancy === '' ? null : Number(form.included_occupancy);
...
const existingNumbers = computed(
    () => new Set(props.rooms.map((r) => r.number)),
);
...
const bulkNew = computed(
    () => bulkPreview.value.filter((p) => !p.exists).length,
);
const bulkOverLimit = computed(
    () =>
        props.maxRooms !== null &&
        props.rooms.length + bulkNew.value > props.maxRooms,
);
...
async function submitBulk() {
    ...
    try {
        ...
        toast.success(
            ...
            data.skipped.length
                ? `Se omitieron ${data.skipped.length} números que ya existían: ${data.skipped.join(', ')}.`
                : undefined,
        );
...
async function submitQuick() {
    ...
    try {
        await axios.post('/api/rooms/single-unit', {
            ...
            duration_unit:
                quickForm.rate_type === 'block'
                    ? quickForm.duration_unit
                    : null,
            duration_value:
                quickForm.rate_type === 'block'
                    ? quickForm.duration_value
                    : null,
        });
        showQuick.value = false;
        toast.success(
            'Habitación creada',
            `${quickForm.name} quedó con su tipo, tarifa y habitación ${quickForm.number}.`,
        );
...
async function duplicateRoom(room: RoomRow) {
    try {
        const { data } = await axios.post<{ number: string }>(
            `/api/rooms/${room.id}/duplicate`,
        );
        toast.success(
            'Habitación duplicada',
            `Se creó la ${data.number} con la misma ficha que la ${room.number}.`,
        );
        router.reload({ only: ['rooms'] });
    } catch (error) {
        handleError(error);
    ...
    try {
        if (editing.value) {
            ...
            toast.success(
                ...
                `La habitación ${form.number} se guardó correctamente.`,
            );
        } else {
            ...
            toast.success(
                'Habitación creada',
                `La habitación ${form.number} se agregó al plano.`,
            );
        }
...
async function submitDelete() {
    ...
    try {
        ...
        toast.success(
            ...
            `La habitación ${number} se quitó del sistema.`,
        );
        router.reload({ only: ['rooms'] });
    } catch (error) {
        handleError(error);
                ...
                >
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            <h2 class="text-base font-medium">
                                {{
                                    editing
                                        ? `Editar habitación ${editing.number}`
                                        : 'Nueva habitación'
                                }}
                            </h2>
                    ...
                    <div class="flex-1 space-y-7 overflow-y-auto px-6 py-6">
                        ...
                        <div class="space-y-4">
                            ...
                            >
                                <Lucide icon="Hash" class="h-3.5 w-3.5" />
                                Identificación
                            </div>
                            <div class="grid grid-cols-12 gap-4">
                                <div class="col-span-12 sm:col-span-3">
                                    <FormLabel htmlFor="room-number"
                                        >Número *</FormLabel
                                    >
                                    <div class="relative">
                                        <Lucide
                                    ...
                                    <FormHelp
                                        v-if="errors.number"
                                        class="text-danger"
                                        >{{ errors.number }}</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-9">
                                    <FormLabel htmlFor="room-name"
                                        >Nombre comercial (opcional)</FormLabel
                                    >
                                    <div class="relative">
                                        <Lucide
                                            icon="Sparkles"
                                        ...
                                        <FormInput
                                            ...
                                            type="text"
                                            class="pl-9"
                                            placeholder="Suite Luna de Miel"
                                        />
                                    </div>
                                    <FormHelp
                                        v-if="errors.name"
                                ...
                                <div class="col-span-12 sm:col-span-6">
                                    <FormLabel htmlFor="room-zone"
                                        >Zona (opcional)</FormLabel
                                    >
                                    <div class="relative">
                                        <Lucide
                                            icon="Map"
                                ...
                                >
                                    ...
                                    <span>
                                        <span
                                            ...
                                            >Del tipo
                                            {{ selectedFormType.name }}:</span
                                        >
                                        capacidad
                                        {{ selectedFormType.capacity }} pers ·
                                        {{
                                            selectedFormType.price_from !== null
                                                ? `desde $${selectedFormType.price_from}`
                                                : 'sin tarifa (no reservable)'
                                        }}
                                        · horarios y amenidades base — se editan
                                        en
                                        <Link
                                            :href="route('tenant.catalog')"
                                            class="font-medium text-primary hover:underline"
                                            >Zonas y tipos</Link
                                        >. Abajo capturas solo lo propio de ESTA
                                        habitación.
                                    </span>
                                </div>
                            </div>
                        ...
                        <!-- De esta habitación -->
                        <div
                            class="space-y-5 border-t border-slate-200/60 pt-6 dark:border-darkmode-400"
                        >
                            <div
                                class="flex items-center gap-2 text-xs font-medium tracking-wide text-slate-400 uppercase"
                            >
                            ...
                            <div>
                                <FormLabel htmlFor="room-description"
                            ...
                            <div>
                                ...
                                <div
                                    class="rounded-lg border border-dashed border-slate-300/70 p-3 dark:border-darkmode-400"
                                >
                                    <div
                                        v-if="form.beds.length"
                                        class="space-y-2.5"
                                    >
                                        <div
                                        ...
                                        >
                                            <div
                                                class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-400 dark:bg-darkmode-400"
                                            >
                                                <Lucide
                                                    icon="BedDouble"
                                                    class="h-4 w-4"
                                    ...
                                    <div
                                        v-else
                                        class="flex items-center justify-center gap-2 py-2 text-xs text-slate-400"
                                    >
                                        <Lucide
                                            icon="BedDouble"
                                            class="h-4 w-4"
                                        />
                                        Sin camas configuradas — se hereda la
                                        capacidad del tipo.
                                    </div>
                                </div>
                                <FormHelp
                            ...
                            <div class="grid grid-cols-12 gap-x-5 gap-y-4">
                                <div class="col-span-12 sm:col-span-6">
                                    <FormLabel htmlFor="room-size"
                                        >Superficie (m²)</FormLabel
                                    >
                                    <div class="relative">
                                        <Lucide
                                            icon="Ruler"
                                    ...
                                    <FormHelp
                                        v-if="errors.size_m2"
                                        class="text-danger"
                                        >{{ errors.size_m2 }}</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-6">
                                    <FormLabel htmlFor="room-view"
                                        >Vista</FormLabel
                                    >
                                    <div class="relative">
                                        <Lucide
                            ...
                            <!-- Amenidades -->
                            <div>
                                <FormLabel htmlFor="room-amenities"
                                    >Amenidades</FormLabel
                                >
                                <div class="relative">
                                    <Lucide
                                        icon="Sparkles"
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <button
                                            type="button"
                                            class="text-primary/50 hover:text-danger"
                                            title="Quitar"
                            ...
                            <div class="grid gap-3 sm:grid-cols-2">
                                <div
                                    class="flex items-center justify-between gap-3 rounded-lg border border-slate-200/70 p-3 dark:border-darkmode-400"
                                >
                                    <span
                        ...
                        >
                            <div
                                class="flex items-center gap-2 text-xs font-medium tracking-wide text-slate-400 uppercase"
                            >
                                <Lucide icon="DollarSign" class="h-3.5 w-3.5" />
                                Ajuste de precio de esta habitación
                            </div>
                            <div class="max-w-sm">
                                <FormLabel htmlFor="room-price-modifier"
                                    >Ajuste sobre la tarifa del tipo
                                    ($)</FormLabel
                                >
                                <div class="relative">
                                    <Lucide
                                    ...
                                    />
                                    <FormInput
                                        id="room-price-modifier"
                                        v-model="form.price_modifier"
                                        type="number"
                                        step="0.01"
                                        placeholder="0"
                                        class="pl-9"
                                ...
                                <FormHelp>
                                    NO es por número de personas — es por
                                    características de ESTE cuarto: +100 vista
                                    al mar, −50 interior. Se suma a la tarifa
                                    por cada noche/periodo que se cobre.
                                </FormHelp>
                                <FormHelp
                                    v-if="errors.price_modifier"
                                    class="text-danger"
                                    >{{ errors.price_modifier }}</FormHelp
                                >
                            </div>
                        </div>
                        ...
                        >
                            ...
                            >
                                ...
                                Ocupación de esta habitación
                            </div>
                            <p class="text-xs text-slate-500">
                                El wizard y las reservas nunca aceptan más
                                personas que la "ocupación máxima" de aquí abajo
                                — si la dejas vacía, el techo es la capacidad
                                del tipo. Entre las incluidas y el máximo, cada
                                persona de más paga el cobro extra.
                            </p>
                            <div class="grid grid-cols-12 gap-x-5 gap-y-4">
                                <div class="col-span-12 sm:col-span-4">
                                    <FormLabel htmlFor="room-max-occupancy"
                                        >Ocupación máxima</FormLabel
                                    >
                                    <div class="relative">
                                        <Lucide
                                            icon="UsersRound"
                                        ...
                                        <FormInput
                                            ...
                                            min="1"
                                            class="pl-9"
                                            placeholder="hereda del tipo"
                                        />
                                    </div>
                                    <FormHelp>
                                        El techo real de este cuarto. Vacío =
                                        hereda la capacidad del tipo ({{
                                            selectedFormType?.capacity ?? '—'
                                        }}).
                                    </FormHelp>
                                    <FormHelp
                                        v-if="errors.max_occupancy"
                                        class="text-danger"
                                        >{{ errors.max_occupancy }}</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-4">
                                    <FormLabel htmlFor="room-included-occupancy"
                                    ...
                                    <div class="relative">
                                        ...
                                        />
                                        <FormInput
                                            id="room-included-occupancy"
                                            v-model="form.included_occupancy"
                                            type="number"
                                            min="1"
                                            class="pl-9"
                                            placeholder="2"
                                    ...
                                    <FormHelp>
                                        Vacío = sin cobro por persona extra, sin
                                        importar cuánta gente se reserve.
                                    </FormHelp>
                                    <FormHelp
                                        v-if="errors.included_occupancy"
                                        class="text-danger"
                                        >{{
                                            errors.included_occupancy
                                        }}</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-4">
                                    <FormLabel htmlFor="room-extra-guest-fee"
                                        >Cobro por cada persona extra
                                        ($)</FormLabel
                                    >
                                    <div class="relative">
                                        <Lucide
                                    ...
                                    <FormHelp>
                                        Por persona que exceda las incluidas,
                                        por noche o periodo de la tarifa.
                                    </FormHelp>
                                    <FormHelp
                                        v-if="errors.extra_guest_fee"
                                        class="text-danger"
                                        >{{ errors.extra_guest_fee }}</FormHelp
                                    >
                                </div>
                            </div>
                            <div
                                ...
                                :class="
                                    occupancyPreview.unreachable
                                        ? 'border border-warning/30 bg-warning/5 text-slate-600 dark:text-slate-300'
                                        : 'bg-slate-50 text-slate-600 dark:bg-darkmode-400 dark:text-slate-300'
                                "
                            >
                                <Lucide
                                    :icon="
                            ...
                            <div>
                                ...
                                >
                                    <div
                                        v-if="form.optional_charges.length"
                                        class="space-y-2.5"
                                    >
                                        <div
                                            v-for="(
                                                charge, index
                                            ) in form.optional_charges"
                                            :key="index"
                                            class="flex items-center gap-2.5"
                                        >
                                            <div
                                            ...
                                            <FormInput
                                                ...
                                                maxlength="100"
                                                class="flex-1"
                                                placeholder="Mascota, decoración, cama extra…"
                                            />
                                            <div class="relative w-32">
                                                <Lucide
                                                    icon="DollarSign"
                                                    class="absolute inset-y-0 left-0 z-10 my-auto ml-2.5 h-3.5 w-3.5 stroke-[1.3] text-slate-400"
                                                />
                                                <FormInput
                                    ...
                                    >
                                        <Lucide
                                            ...
                                            class="h-4 w-4"
                                        />
                                        Sin cargos opcionales — agrégalos y el
                                        personal podrá aplicarlos al reservar o
                                        en walk-in.
                                    </div>
                                </div>
                                <FormHelp
                                    v-if="errors.optional_charges"
                                    class="text-danger"
                                    >{{ errors.optional_charges }}</FormHelp
                                >
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <div class="grid grid-cols-12 gap-4">
                                <div class="col-span-12 sm:col-span-6">
                                    <FormLabel htmlFor="room-notes"
                                        >Notas internas</FormLabel
                                    >
                                    <FormTextarea
                                        id="room-notes"
                                        v-model="form.notes"
                                        placeholder="Aire nuevo, TV pendiente…"
                                    />
                                    <FormHelp
                                        v-if="errors.notes"
                                        class="text-danger"
                                        >{{ errors.notes }}</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-6">
                                    <FormLabel htmlFor="room-maintenance-notes"
                                        >Notas de mantenimiento</FormLabel
                                    >
                                    <FormTextarea
                                    ...
                                    />
                                    <FormHelp
                                        >Notas de mantenimiento — por qué está
                                        fuera de servicio.</FormHelp
                                    >
                                    <FormHelp
                                        v-if="errors.maintenance_notes"
                                        class="text-danger"
                                        >{{
                ...
                >
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 text-xs text-slate-500">
                                Varias habitaciones del mismo tipo y zona de un
                                jalón.
                            </p>
                        </div>
                        <button
                    ...
                    <div class="flex-1 space-y-4 overflow-y-auto px-6 py-5">
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-12 sm:col-span-6">
                                <FormLabel htmlFor="bulk-type"
                                    >Tipo *</FormLabel
                                >
                                <FormSelect
                                    id="bulk-type"
                                    v-model="bulkForm.room_type_id"
                                >
                                    <option
                                        v-for="type in roomTypes"
                                        :key="type.id"
                            ...
                            <div class="col-span-12 sm:col-span-6">
                                <FormLabel htmlFor="bulk-zone"
                                    >Zona (opcional)</FormLabel
                                >
                                <FormSelect
                                    id="bulk-zone"
                                    v-model="bulkForm.zone_id"
                                >
                                    <option value="">Sin zona</option>
                                    <option
                                        v-for="zone in zones"
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                <FormLabel htmlFor="bulk-from"
                                    >Del número *</FormLabel
                                >
                                <FormInput
                                    id="bulk-from"
                                    v-model="bulkForm.number_from"
                                    type="number"
                                    min="1"
                                    placeholder="101"
                                />
                                <FormHelp
                                    v-if="errors.number_from"
                                    class="text-danger"
                                    >{{ errors.number_from }}</FormHelp
                                >
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <FormLabel htmlFor="bulk-to"
                                    >Al número *</FormLabel
                                >
                                <FormInput
                                    id="bulk-to"
                                    v-model="bulkForm.number_to"
                                    type="number"
                                    min="1"
                                    placeholder="110"
                                />
                                <FormHelp
                                    v-if="errors.number_to"
                                    class="text-danger"
                                    >{{ errors.number_to }}</FormHelp
                                >
                            </div>
                        </div>
                        ...
                        >
                            <div class="mb-2.5 text-xs text-slate-500">
                                ...
                                <template v-if="bulkPreview.length - bulkNew">
                                    <span class="font-medium text-warning">{{
                                        bulkPreview.length - bulkNew
                                    }}</span>
                                    ya existen y se omiten.
                                </template>
                                <template v-else
                                    >ningún número está ocupado.</template
                                >
                            </div>
                            <div
                            ...
                            >
                                <span
                                    ...
                                    :class="
                                        item.exists
                                            ? 'bg-slate-100 text-slate-400 line-through dark:bg-darkmode-400'
                                            : 'bg-success/10 text-success'
                                    "
                                    :title="
                                        item.exists
                        ...
                        >
                            ...
                            />
                            Este rango rebasa el límite de tu plan ({{
                                rooms.length
                            }}
                            de {{ maxRooms }} usadas). Ajusta el rango o revisa
                            <Link
                                :href="route('tenant.hotel-settings')"
                    ...
                    >
                        ...
                        <Button
                            ...
                            variant="primary"
                            class="shadow-md shadow-primary/20"
                            :disabled="saving || !bulkNew || bulkOverLimit"
                        >
                            <Lucide icon="Check" class="mr-2 h-4 w-4" />
                            {{
                                saving
                                    ? 'Creando…'
                                    : `Crear ${bulkNew || ''} habitación(es)`
                            }}
                        </Button>
                    </div>
                </form>
            ...
            <Dialog.Panel>
                ...
                >
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            <h2 class="text-base font-medium">
                                Alta rápida: habitación única
                            </h2>
                            <p class="mt-0.5 text-xs text-slate-500">
                                Crea el tipo, su tarifa base y la habitación en
                                un solo paso — para habitaciones que no se
                                repiten.
                            </p>
                        </div>
                        <button
                    ...
                    <div class="flex-1 space-y-4 overflow-y-auto px-6 py-5">
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-12 sm:col-span-3">
                                <FormLabel htmlFor="quick-number"
                                    >Número *</FormLabel
                                >
                                <FormInput
                                    id="quick-number"
                                    v-model="quickForm.number"
                                    type="text"
                                    placeholder="501"
                                />
                                <FormHelp
                            ...
                            <div class="col-span-12 sm:col-span-9">
                                ...
                                <FormInput
                                    ...
                                    type="text"
                                    placeholder="Habitación Master Junior VIP"
                                />
                                <FormHelp
                                    v-if="errors.name"
                                    class="text-danger"
                                ...
                                >
                                <FormHelp v-else
                                    >Se usa como nombre del tipo y de la
                                    habitación.</FormHelp
                                >
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <FormLabel htmlFor="quick-capacity"
                                    >Capacidad (pax)</FormLabel
                                >
                                <FormInput
                                    id="quick-capacity"
                                    v-model.number="quickForm.capacity"
                                    type="number"
                                    min="1"
                                    max="20"
                                />
                            ...
                            <div class="col-span-6 sm:col-span-9">
                                <FormLabel htmlFor="quick-zone"
                                    >Zona (opcional)</FormLabel
                                >
                                <FormSelect
                                    id="quick-zone"
                                    v-model="quickForm.zone_id"
                                >
                                    <option value="">Sin zona</option>
                                    <option
                                        v-for="zone in zones"
                        ...
                        >
                            <div
                                class="mb-3 flex items-center gap-1.5 text-sm font-medium"
                            >
                                <Lucide
                                    icon="Tag"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <FormLabel htmlFor="quick-rate-type"
                                        >Cobro</FormLabel
                                    >
                                    <FormSelect
                                        id="quick-rate-type"
                                        v-model="quickForm.rate_type"
                                    >
                            ...
                            <div
                                v-if="quickForm.rate_type === 'block'"
                                class="mt-3"
                            >
                                ...
                                <div class="flex gap-2">
                                    ...
                                    />
                                    <FormSelect
                                        v-model="quickForm.duration_unit"
                                        class="flex-1"
                                    >
                                        <option
                            ...
                            <FormHelp class="mt-2"
                                >Queda como la tarifa "Tarifa base"; camas,
                                vista y demás detalles se completan después
                                editando la habitación.</FormHelp
                            >
                        </div>
        ...
        <Dialog :open="deleting !== null" @close="deleting = null">
            <Dialog.Panel>
                <div class="p-5 text-center">
                    ...
                    />
                    <h2 class="text-base font-medium">
                        ¿Eliminar la habitación {{ deleting?.number }}?
                    </h2>
                    <p class="mt-2 text-sm text-slate-500">
                        Se quitará del plano y del sistema.
                    </p>
