...
function submitZone() {
    const payload = {
        ...
        sort_order:
            zoneForm.sort_order === '' ? 0 : Number(zoneForm.sort_order),
    };
    mutate(
        () =>
            editingZone.value
...
async function uploadPhotos(event: Event) {
    ...
    try {
        const { data } = await axios.post<{ photos: TypePhoto[] }>(
            `/api/room-types/${editingType.value.id}/photos`,
            formData,
        );
        ...
        toast.error(
            ...
            e.response?.data?.message ??
                (errs
                    ...
                    : 'Revisa formato (JPG, PNG, WebP) y peso (máx. 6 MB).'),
        );
    } finally {
...
async function removePhoto(photo: TypePhoto) {
    ...
    photoBusy.value = true;
    try {
        const { data } = await axios.delete<{ photos: TypePhoto[] }>(
            `/api/room-types/${editingType.value.id}/photos/${photo.id}`,
        );
        typePhotos.value = data.photos;
        router.reload({ only: ['roomTypes'] });
    } catch {
...
async function makeCover(photo: TypePhoto) {
    ...
    try {
        ...
        const { data } = await axios.patch<{ photos: TypePhoto[] }>(
            `/api/room-types/${editingType.value.id}/photos/order`,
            { order },
        );
        typePhotos.value = data.photos;
...
function submitType() {
    const payload: Record<string, unknown> = {
        ...
        max_children:
            typeForm.max_children === '' ? null : Number(typeForm.max_children),
        check_in_time:
            typeForm.check_in_time === '' ? null : typeForm.check_in_time,
        check_out_time:
    ...
    if (!editingType.value) {
        ...
        payload.duration_unit =
            typeForm.rate_type === 'block' ? typeForm.duration_unit : null;
        payload.duration_value =
            typeForm.rate_type === 'block' ? typeForm.duration_value : null;
    }
    ...
    mutate(
        () =>
            editingType.value
                ? axios.patch(
                      `/api/room-types/${editingType.value.id}`,
                      payload,
                  )
                : axios.post('/api/room-types', {
                      ...payload,
                      property_id: props.property.id,
                  }),
        editingType.value
            ? 'Tipo de habitación actualizado'
            : 'Tipo creado con su tarifa base',
...
function duplicateType(type: RoomTypeRow) {
    mutate(
        () => axios.post(`/api/room-types/${type.id}/duplicate`),
        'Tipo duplicado con sus tarifas',
    );
}
...
function openPlan(plan: RatePlanRow | null) {
    ...
    planForm.cancel_penalty_percent = plan?.cancel_penalty_percent
...
function submitPlan() {
    const payload: Record<string, unknown> = {
        ...
        type: planForm.type,
        duration_unit:
            planForm.type === 'block' ? planForm.duration_unit : null,
        duration_value:
            planForm.type === 'block' ? planForm.duration_value : null,
        price: planForm.price,
        min_advance_unit: planForm.has_advance
            ? planForm.min_advance_unit
            : null,
        ...
        payment_due_unit: planForm.has_prepayment
            ...
            : null,
        payment_due_value: planForm.has_prepayment
            ? planForm.payment_due_value
            : null,
        cancel_free_unit: planForm.has_cancel_policy
            ? planForm.cancel_free_unit
            : null,
        cancel_free_value: planForm.has_cancel_policy
            ? planForm.cancel_free_value
            : null,
        cancel_penalty_percent: planForm.has_cancel_policy
    ...
    mutate(
        () =>
            editingPlan.value
                ? axios.patch(
                      `/api/rate-plans/${editingPlan.value.id}`,
                      payload,
                  )
                : axios.post('/api/rate-plans', {
                      ...payload,
                      property_id: props.property.id,
                  }),
...
async function openSeasons(plan: RatePlanRow) {
    ...
    try {
        const { data } = await axios.get<SeasonRow[]>(
            `/api/rate-plans/${plan.id}/seasons`,
        );
        seasons.value = data;
    } catch {
...
function submitSeason() {
    ...
    mutate(
        () =>
            editingSeason.value
                ? axios.patch(
                      `/api/rate-plans/${planId}/seasons/${editingSeason.value.id}`,
                      payload,
                  )
...
async function deleteSeason(season: SeasonRow) {
    ...
    const planId = seasonsPlan.value.id;
    await mutate(
        () => axios.delete(`/api/rate-plans/${planId}/seasons/${season.id}`),
        'Temporada eliminada',
    );
            ...
            <div class="box box--stacked mt-5 overflow-hidden">
                ...
                >
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-xs text-slate-500">
                            Config&shy;úralo en 3 pasos. Sobre esto se apoyan
                            tus habitaciones y reservas.
                        </p>
                    </div>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        <Lucide
                            :icon="showGuide ? 'ChevronUp' : 'ChevronDown'"
                            class="h-3.5 w-3.5"
                        />
                    </button>
                </div>
                <div v-if="showGuide" class="grid gap-4 p-5 md:grid-cols-3">
                    ...
                    >
                        ...
                        <p
                            class="mt-2.5 text-xs leading-relaxed text-slate-500"
                        >
                            <span
                                class="font-medium text-slate-600 dark:text-slate-300"
                                >Dónde</span
                            >
                            están las habitaciones. Agrupas por ubicación para
                            el plano. Ej:
                            <span class="font-medium">Planta baja</span>,
                            <span class="font-medium">Piso 1</span>.
                        </p>
                        <div
                            class="mt-auto flex items-center gap-1.5 pt-2.5 text-xs"
                            :class="
                                zones.length ? 'text-success' : 'text-slate-400'
                            "
                        >
                            <Lucide
                                :icon="zones.length ? 'CircleCheck' : 'Circle'"
                                class="h-3.5 w-3.5"
                            />
                            {{
                                zones.length
                                    ? `${zones.length} zona(s) creada(s)`
                                    : 'Pendiente: crea la primera zona'
                            }}
                        </div>
                    </div>
                    ...
                    >
                        <div class="flex items-center gap-2.5">
                            <div
                                class="flex h-8 w-8 items-center justify-center rounded-full bg-info/10 text-sm font-semibold text-info"
                            >
                                2
                            </div>
                            <div class="flex items-center gap-1.5 font-medium">
                        ...
                        >
                            <span
                                class="font-medium text-slate-600 dark:text-slate-300"
                                >Qué ofreces y cómo lo cobras.</span
                            >
                            Al crear un tipo capturas su precio UNA vez (queda
                            como su tarifa base); luego puedes agregar más
                            tarifas: rato, semana, promo.
                        </p>
                        <div
                            class="mt-auto flex items-center gap-1.5 pt-2.5 text-xs"
                            :class="
                    ...
                    >
                        <div class="flex items-center gap-2.5">
                            <div
                                class="flex h-8 w-8 items-center justify-center rounded-full bg-success/10 text-sm font-semibold text-success"
                            >
                                3
                            </div>
                            <div class="flex items-center gap-1.5 font-medium">
                        ...
                        >
                            ...
                            <span
                                class="font-medium text-slate-600 dark:text-slate-300"
                                >reales</span
                            >, cada una con su zona y su tipo. Se dan de alta en
                            <Link
                        ...
                        <div
                            class="mt-auto flex items-center gap-1.5 pt-2.5 text-xs"
                            :class="
                                totalRooms ? 'text-success' : 'text-slate-400'
                            "
                        >
                            <Lucide
                                :icon="totalRooms ? 'CircleCheck' : 'Circle'"
                                class="h-3.5 w-3.5"
                            />
                            <template v-if="totalRooms"
                                >{{ totalRooms }} habitación(es) dada(s) de
                                alta</template
                            >
                            <template v-else>
                                Pendiente:&nbsp;
                    ...
                    <div class="box box--stacked flex flex-1 flex-col">
                        ...
                        >
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">
                                        Qué ofreces y cómo lo cobras — el precio
                                        vive en las tarifas
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="flex-1">
                            ...
                            >
                                <div v-for="type in roomTypes" :key="type.id">
                                    ...
                                    >
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            >
                                                ...
                                                >
                                                · {{ type.rooms_count }} hab. ·
                                                {{
                                                    plansForType(type.id).length
                                                }}
                                                tarifa(s)
                                            </div>
                                        </div>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            />
                                            Este tipo no tiene tarifas: no se
                                            puede reservar.
                                            <Button
                                                v-if="canManage"
                                                variant="outline-primary"
                                ...
                                <div
                                    v-if="orphanPlans.length"
                                    class="px-5 py-3.5"
                                >
                                    <div
                                        class="mb-2 text-xs font-medium tracking-wide text-slate-400 uppercase"
                                    >
                                        Tarifas sin tipo asignado
                                    </div>
                                    <div
                                        v-for="plan in orphanPlans"
                                        :key="plan.id"
                            ...
                            >
                                ...
                                <div>
                                    <p class="text-sm font-medium">
                                        Aún no hay tipos de habitación
                                    </p>
                                    <p class="mt-0.5 text-xs text-slate-500">
                                        Ej: "Sencilla", "Suite", "Jacuzzi". Al
                                        crearlo capturas su precio una sola vez.
                                    </p>
                                </div>
                                <Button
                                    v-if="canManage"
                ...
                <div class="p-5">
                    ...
                    <form class="space-y-4" @submit.prevent="submitPlan">
                        ...
                        <div v-if="planForm.type === 'block'">
                            ...
                            <FormHelp
                                >Ej: 3 horas (rato), 1 día, 1 semana, 1 mes. El
                                precio es por cada periodo completo.</FormHelp
                            >
                            <FormHelp
                                v-if="
                        ...
                        >
                            <FormCheck>
                                <FormCheck.Input
                                    id="plan-advance"
                                    v-model="planForm.has_advance"
                                    type="checkbox"
                                />
                                <FormCheck.Label htmlFor="plan-advance"
                                    >Exigir antelación mínima para
                                    reservar</FormCheck.Label
                                >
                            </FormCheck>
                            <div v-if="planForm.has_advance" class="mt-3">
                                <div class="flex gap-2">
                                    ...
                                    />
                                    <FormSelect
                                        v-model="planForm.min_advance_unit"
                                        class="flex-1"
                                    >
                                        <option
                                            v-for="unit in advanceUnits"
                                            :key="unit.value"
                                            :value="unit.value"
                                        >
                                            {{ unit.label }}
                                ...
                                <FormHelp
                                    >Ej: 4 horas → solo se aceptan reservas
                        ...
                        >
                            <FormCheck>
                                <FormCheck.Input
                                    id="plan-prepay"
                                    v-model="planForm.has_prepayment"
                                    type="checkbox"
                                />
                                <FormCheck.Label htmlFor="plan-prepay"
                                    >Exigir cobro anticipado</FormCheck.Label
                                >
                            </FormCheck>
                            <div
                                v-if="planForm.has_prepayment"
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        />
                                        <FormSelect
                                            v-model="planForm.payment_due_unit"
                                            class="flex-1"
                                        >
                                            <option
                                                v-for="unit in advanceUnits"
                                    ...
                                    <FormHelp
                                        >Ej: 1 semana → el total debe estar
                                        pagado una semana antes de la llegada;
                                        si no, la reserva se marca "pago
                                        vencido".</FormHelp
                                    >
                                    <FormHelp
                                        v-if="
                                            errors.payment_due_value ||
                                            errors.payment_due_unit
                                        "
                                        class="text-danger"
                                    >
                                        {{
                        ...
                        >
                            <FormCheck>
                                <FormCheck.Input
                                    ...
                                    v-model="planForm.has_cancel_policy"
                                    type="checkbox"
                                />
                                <FormCheck.Label htmlFor="plan-cancel-policy"
                                    >Política de cancelación con
                                    reembolso</FormCheck.Label
                                >
                            </FormCheck>
                            <div
                                v-if="planForm.has_cancel_policy"
                                class="mt-3 space-y-3"
                            >
                                <div>
                                    <FormLabel
                                        >Cancelación sin costo hasta</FormLabel
                                    >
                                    <div class="flex gap-2">
                                        <FormInput
                                            v-model.number="
                                        ...
                                        />
                                        <FormSelect
                                            v-model="planForm.cancel_free_unit"
                                            class="flex-1"
                                        >
                                            <option
                                                v-for="unit in advanceUnits"
                                    ...
                                    <FormHelp
                                        >Antes de este límite se sugiere
                                        reembolsar todo lo pagado.</FormHelp
                                    >
                                    <FormHelp
                                        v-if="
                                ...
                                <div>
                                    <FormLabel htmlFor="plan-cancel-penalty"
                                        >Penalidad fuera de la ventana (% de lo
                                        pagado que se retiene)</FormLabel
                                    >
                                    <FormInput
                                        id="plan-cancel-penalty"
                                        v-model.number="
                                            planForm.cancel_penalty_percent
                                        "
                                        type="number"
                                    ...
                                    />
                                    <FormHelp
                                        >100 = sin reembolso al cancelar tarde
                                        (lo típico). Es una sugerencia al
                                        reembolsar; tu equipo siempre
                                        decide.</FormHelp
                                    >
                                    <FormHelp
                                        v-if="errors.cancel_penalty_percent"
                                        class="text-danger"
                                        >{{
                        ...
                        <FormCheck v-if="editingPlan">
                            <FormCheck.Input
                                ...
                                type="checkbox"
                            />
                            <FormCheck.Label htmlFor="plan-active"
            ...
            <Dialog.Panel>
                <div class="p-5">
                    ...
                    <p class="mb-4 text-xs text-slate-500">
                        {{ seasonsPlan?.name }} — un rango de fechas con un
                        precio que SUSTITUYE al de la tarifa mientras esté
                    ...
                    <template v-else>
                            ...
                            >
                                ...
                                <div class="flex shrink-0 gap-3">
                                    <a
                                        ...
                                        class="text-primary"
                                        title="Editar"
                                        @click.prevent="editSeason(season)"
                                    >
                                    ...
                                    <a
                                        ...
                                        class="text-danger"
                                        title="Eliminar"
                                        @click.prevent="deleteSeason(season)"
                                    >
                        ...
                        <div
                            v-else
                            class="mb-5 rounded-md border border-dashed border-slate-200 px-4 py-3 text-center text-xs text-slate-500 dark:border-darkmode-400"
                        >
                            Sin temporadas — la tarifa cobra su precio normal
                            todo el año.
                        </div>
                        ...
                        >
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <FormHelp
                                        >Sustituye el precio normal ({{
                                            money(seasonsPlan?.price ?? 0)
                                        }}) mientras esté vigente.</FormHelp
                                    >
                                    <FormHelp
                                        v-if="errors.price"
                                        class="text-danger"
                                ...
                                <div>
                                    <FormLabel htmlFor="season-priority"
                                        >Prioridad</FormLabel
                                    >
                                    <FormInput
                                        id="season-priority"
            ...
            <Dialog.Panel>
                <div class="p-5">
                    ...
                    <form class="space-y-4" @submit.prevent="submitZone">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <FormLabel htmlFor="zone-kind"
                                    >Tipo de zona</FormLabel
                                >
                                <FormSelect
                                    id="zone-kind"
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <Button
                                    ...
                                    variant="outline-secondary"
                                    size="sm"
                                    @click="zoneForm.color = null"
                                >
                                    Sin color
                                </Button>
                                <span class="text-xs text-slate-500">{{
                                    zoneForm.color ?? 'Sin color asignado'
                                }}</span>
                            </div>
                            <FormHelp
                                >Se usa como puntito identificador en el plano y
                                las listas.</FormHelp
                            >
                            <FormHelp v-if="errors.color" class="text-danger">{{
                                errors.color
                            }}</FormHelp>
                        </div>
                        <div class="flex justify-end gap-2 pt-2">
                    ...
                    >
                        ...
                        <section>
                            ...
                            <div class="grid grid-cols-12 gap-5">
                                ...
                                <div class="col-span-12">
                                    <FormLabel htmlFor="type-description"
                        ...
                        >
                            ...
                            <p class="mb-4 text-xs text-slate-500">
                                La galería que ve el huésped en el wizard. La
                                primera foto es la portada de la tarjeta.
                            </p>
                            ...
                            >
                                Guarda el tipo primero; después podrás subir sus
                                fotos desde aquí.
                            </div>
                            <template v-else>
                                <div
                                    v-if="typePhotos.length"
                                ...
                                <div
                                    ...
                                    class="rounded-lg border border-dashed border-slate-300/70 px-4 py-5 text-center text-xs text-slate-500 dark:border-darkmode-400"
                                >
                                    Sin fotos: la tarjeta del wizard se muestra
                                    solo con texto. Una buena galería es lo que
                                    más ayuda a que reserven.
                                </div>
                        ...
                        >
                            ...
                            <div class="grid grid-cols-12 gap-5">
                                <div class="col-span-12 sm:col-span-4">
                                    ...
                                    <FormInput
                                        id="type-capacity"
                                        v-model.number="typeForm.capacity"
                                        type="number"
                                        min="1"
                                        max="20"
                                    />
                                    <FormHelp
                                        ...
                                        class="text-danger"
                                        >{{ errors.capacity }}</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-4">
                                    <FormLabel htmlFor="type-max-adults"
                                        >Máx. adultos</FormLabel
                                    >
                                    <FormInput
                                        id="type-max-adults"
                                        v-model="typeForm.max_adults"
                                        type="number"
                                        min="0"
                                        placeholder="—"
                                    />
                                    <FormHelp
                                        ...
                                        class="text-danger"
                                        >{{ errors.max_adults }}</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-4">
                                    <FormLabel htmlFor="type-max-children"
                                        >Máx. niños</FormLabel
                                    >
                                    <FormInput
                                        id="type-max-children"
                                        v-model="typeForm.max_children"
                                        type="number"
                                        min="0"
                                        placeholder="—"
                                    />
                        ...
                        >
                            ...
                            <div class="grid grid-cols-12 gap-5">
                                ...
                                <div class="col-span-12 sm:col-span-8">
                                    <FormLabel htmlFor="type-rate-type"
                                        >Cobro</FormLabel
                                    >
                                    <FormSelect
                                        id="type-rate-type"
                                ...
                                >
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        />
                                        <FormSelect
                                            v-model="typeForm.duration_unit"
                                            class="flex-1"
                                        >
                                            <option
                                                v-for="unit in durationUnits"
                            ...
                            <FormHelp class="mt-2">
                                Se guarda como la tarifa "Tarifa base" del tipo.
                                Después puedes agregar más tarifas (rato,
                                semana, promo) desde la lista.
                            </FormHelp>
                        </section>
                        <section
                            v-else
                        ...
                        >
                            ...
                            >
                                ...
                                />
                                <span>
                                    El precio de este tipo vive en sus
                                    <span class="font-medium">tarifas</span>: se
                                    editan en su fila de la lista (expande el
                                    tipo) o agrega una nueva con el icono de
                                    etiqueta.
                                </span>
                            </div>
                        </section>
                        ...
                        >
                            ...
                            <div class="grid grid-cols-12 gap-5">
                                <div class="col-span-12 sm:col-span-4">
                                    ...
                                    >
                                    <FormHelp v-else
                                        >Menor número aparece primero.</FormHelp
                                    >
                                </div>
                                <label
