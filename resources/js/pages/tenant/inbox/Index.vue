        ...
        <div class="mt-2">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-slate-500">
                        {{ property.name }} · todos los canales en un solo lugar
                    </p>
                </div>
                <Button
                    as="a"
            ...
            <div class="mt-5 grid grid-cols-12 gap-3">
                ...
                >
                    ...
                    >
                        <Lucide
                            :icon="
                                channelMeta[ch.type]?.icon ?? 'MessageCircle'
                            "
                            class="h-4 w-4"
                        />
                    </div>
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="truncate text-xs text-slate-500">
                            {{ channelMeta[ch.type]?.label ?? ch.type }}
                        </div>
                    </div>
                    <FormSelect
                        v-if="canManage"
                    ...
                    <span
                        v-else
                        class="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500 dark:bg-darkmode-400"
                    >
            ...
            <div class="mt-5 grid grid-cols-12 gap-6">
                ...
                <div class="col-span-12 xl:col-span-4">
                    <div
                        class="box box--stacked flex h-[calc(100vh-21rem)] min-h-[480px] flex-col"
                    >
                        <!-- Filtros -->
                        <div
                            class="border-b border-slate-200/60 p-3 dark:border-darkmode-400"
                        >
                            <div
                        ...
                        <!-- Conversaciones -->
                        <div
                            class="flex-1 divide-y divide-slate-100 overflow-y-auto dark:divide-darkmode-400/60"
                        >
                            <div
                                v-for="c in filtered"
                                :key="c.id"
                            ...
                            >
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    >
                                        ...
                                        <span
                                            v-if="c.unread"
                                            class="ml-auto flex h-4 min-w-4 shrink-0 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-semibold text-white"
                                            >{{ c.unread }}</span
                                        >
                                    </div>
                                    <div
                                    ...
                                    >
                                        <span
                                            ...
                                            :class="statusMeta[c.status]?.tone"
                                            >{{
                                        ...
                                        <span
                                            ...
                                            "
                                            class="rounded-full bg-pending/10 px-1.5 py-0.5 text-[10px] font-medium text-pending"
                                            >Verificar pago</span
                                        >
                                        <span
                                            v-else-if="
                                        ...
                                        <button
                                            v-if="canManage"
                                            type="button"
                                            class="ml-auto rounded p-1 text-slate-400 opacity-0 transition group-hover:opacity-100 hover:bg-danger/10 hover:text-danger focus:opacity-100"
                            ...
                            >
                                ...
                                <p class="px-6 text-sm text-slate-500">
                                    Sin conversaciones{{
                                        filter !== 'all'
                                            ? ' en este filtro'
                                            : ' todavía. Comparte el webchat de tu hotel para empezar'
                                    }}.
                                </p>
                            </div>
                        </div>
                ...
                <div class="col-span-12 xl:col-span-8">
                    ...
                    >
                        <template v-if="selected">
                            <!-- Header del hilo -->
                            <div
                                class="flex flex-wrap items-center gap-3 border-b border-slate-200/60 px-5 py-3.5 dark:border-darkmode-400"
                            >
                                <div
                            ...
                            <div
                                ...
                                class="border-b border-dashed border-slate-300/70 bg-primary/[0.03] px-5 py-3.5"
                            >
                                <div class="flex items-start gap-2.5">
                                    <div
                                        class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-primary/10 bg-primary/10"
                                    >
                                        <Lucide
                                            icon="Sparkles"
                            ...
                            <div
                                ref="threadRef"
                                class="flex-1 space-y-3 overflow-y-auto bg-slate-50/60 px-5 py-4 dark:bg-darkmode-700/40"
                            >
                                <div
                                    v-if="threadLoading"
                            ...
                            <div
                                v-if="canManage"
                                class="border-t border-slate-200/60 p-3 dark:border-darkmode-400"
                            >
                                <div
                                    v-if="selected.bot_enabled"
                                    class="mb-2 flex items-center gap-2 rounded-lg bg-primary/5 px-3 py-2 text-xs text-slate-500"
                                >
                                    <Lucide
                                        icon="Bot"
                                        class="h-3.5 w-3.5 text-primary"
                                    ...
                                    respondes tú, la tomas (el bot se pausa).
                                </div>

                                <!-- Copiloto: borrador con aprobación humana -->
                                <div
                                    v-if="suggestLoading"
                                    class="mb-2 flex items-center gap-2 rounded-lg border border-dashed border-primary/30 bg-primary/[0.03] px-3 py-2.5 text-xs text-slate-500"
                                >
                                    <Lucide
                                        icon="Sparkles"
                                ...
                                <div
                                    ...
                                    class="mb-2 rounded-lg border border-primary/20 bg-primary/[0.04] p-3"
                                >
                                    <div
                                    ...
                                    >
                                        ...
                                        <span
                                            class="ml-auto rounded-full bg-primary/10 px-2 py-0.5 text-[10px] tracking-normal text-primary normal-case"
                                            >{{ suggestion.meta.provider }} ·
                                            {{
                                                (
                                                    suggestion.meta.ms / 1000
                                                ).toFixed(1)
                                            }}s</span
                                        >
                                    </div>
                                    <p
                                        class="mt-1.5 text-sm whitespace-pre-line text-slate-600 dark:text-slate-300"
                                    >
                                        {{ suggestion.text }}
                                    </p>
                                    <div class="mt-2.5 flex items-center gap-2">
                                        ...
                                        <button
                                            type="button"
                                            class="ml-auto text-xs text-slate-400 hover:text-danger"
                                ...
                                <div class="flex items-end gap-2">
                                    ...
                                    <textarea
                                        ...
                                        class="max-h-40 min-h-[62px] flex-1 resize-none overflow-y-auto rounded-lg border border-slate-200 px-3.5 py-2.5 text-sm transition outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-darkmode-400 dark:bg-darkmode-600"
                                        @input="autosizeReply"
