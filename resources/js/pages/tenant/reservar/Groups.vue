...
interface GroupHold {
    ...
    experiences: { name: string | null; starts_at: string | null; people: number; total: number }[];
...
async function search() {
    ...
    lines.value = {};
    try {
        const { data } = await axios.get('/api/grupos/availability', { params: dateParams() });
...
function lineFor(option: Option): LineState {
    return (lines.value[option.room_type_id] ??= { rooms: 0, adults: 2, children: 0 });
}
...
function setAdults(option: Option, n: number) {
    const line = lineFor(option);
    line.adults = Math.max(1, Math.min(n, option.effective_capacity - line.children));
}

function setChildren(option: Option, n: number) {
    const line = lineFor(option);
    line.children = Math.max(0, Math.min(n, option.effective_capacity - line.adults));
}

const totalRooms = computed(() => Object.values(lines.value).reduce((sum, l) => sum + l.rooms, 0));
const totalPeople = computed(() => Object.values(lines.value).reduce((sum, l) => sum + l.rooms * (l.adults + l.children), 0));
...
const estimatedTotal = computed(
    () =>
        options.value.reduce((sum, option) => sum + (lines.value[option.room_type_id]?.rooms ?? 0) * option.total, 0)
        + experiencesSubtotal.value,
);

// Resumen del grupo (paso de datos): solo los tipos con habitaciones pedidas.
const selectedOptions = computed(() => options.value.filter((o) => (lines.value[o.room_type_id]?.rooms ?? 0) > 0));

function occupancyLabel(line: { adults: number; children: number }): string {
    const adults = `${line.adults} ${line.adults === 1 ? 'adulto' : 'adultos'}`;
    const children = line.children ? ` + ${line.children} ${line.children === 1 ? 'niño' : 'niños'}` : '';
...
async function loadExperiences() {
    const start = mode.value === 'night' ? arriveDate.value : arriveAt.value.slice(0, 10);
...
async function submitHold() {
    ...
    try {
        ...
        const { data } = await axios.post<GroupHold>('/api/grupos/holds', payload);
        hold.value = data;
        step.value = 'confirm';
        window.scrollTo({ top: 0, behavior: 'smooth' });
        ...
        submitError.value = error.response?.data?.message ?? (errors ? Object.values(errors)[0]?.[0] : null) ?? 'No se pudo apartar el grupo. Intenta de nuevo.';
    } finally {
...
async function preparePayment() {
    ...
    try {
        const { data } = await axios.get<PaymentOptions>('/api/grupos/payment-options');
...
async function requestPayment(method?: 'gateway' | 'transfer', provider?: GatewayOption['provider']) {
    ...
    paymentChoice.value = null;
    try {
        const { data } = await axios.post<PaymentResult>(`/api/grupos/holds/${hold.value.code}/payment`, { method, provider });
        payment.value = data;
        if (data.method === 'gateway' && data.checkout_url) {
            window.location.href = data.checkout_url;
        }
    } catch (error: any) {
        paymentError.value = error.response?.data?.message ?? 'No se pudo generar el cobro. Tu grupo sigue apartado; el hotel te contactará.';
    } finally {
        paymentLoading.value = false;
    }
}
        ...
        <div class="m-auto w-full max-w-4xl">
            <div class="mb-5 flex items-center gap-3 px-1 text-white">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div class="text-xs text-white/70">Reserva grupal · varias habitaciones de un jalón</div>
                </div>
            ...
            <div class="overflow-hidden rounded-2xl bg-white shadow-2xl">
                <!-- ═══ Confirmación / pago ═══ -->
                <div v-if="step === 'confirm' && hold" class="p-5 text-center sm:p-7">
                    ...
                    <h2 class="text-lg font-medium text-slate-800">¡Tu grupo quedó apartado!</h2>
                    ...
                    <div class="mx-auto mt-4 max-w-sm rounded-xl bg-slate-50 p-4 text-left">
                        ...
                        <div v-if="hold.starts_at" class="mt-1 text-xs text-slate-500">
                            {{ formatDateTime(hold.starts_at) }}<template v-if="hold.ends_at"> → {{ formatDateTime(hold.ends_at) }}</template>
                        </div>
                        <div class="mt-3 space-y-1 border-t border-slate-200 pt-3 text-xs text-slate-500">
                            <div v-for="line in hold.rooms" :key="line.room_type" class="flex justify-between">
                                <span>{{ line.rooms }}× {{ line.room_type }}</span><span>{{ money(line.total) }}</span>
                            </div>
                            <div v-for="(line, i) in hold.experiences ?? []" :key="`e-${i}`" class="flex justify-between">
                                <span>{{ line.people }}× {{ line.name }}<template v-if="line.starts_at"> · {{ formatDateTime(line.starts_at) }}</template></span>
                        ...
                        <div class="mt-2 flex items-center justify-between border-t border-slate-200 pt-2 text-base font-medium text-slate-800">
                            <span>Total del grupo</span><span>{{ money(hold.total) }}</span>
                        </div>
                    </div>
                    ...
                    <template v-if="hold.requires_prepayment">
                        ...
                        <template v-else-if="paymentChoice && !payment">
                            <h3 class="mt-5 text-base font-medium text-slate-800">¿Cómo prefieres pagar? Un solo pago por todo el grupo.</h3>
                            ...
                            <p v-if="holdCountdown" class="mt-4 text-xs text-warning">
                                Tu grupo se libera solo en {{ holdCountdown }} si no se completa el pago.
                            </p>
                        </template>

                        <template v-else-if="payment?.method === 'transfer'">
                            <p class="mt-5 text-sm text-slate-600">
                                Transfiere <span class="font-medium">{{ payment.amount_label }}</span> (un solo pago por el grupo) y envía tu comprobante al hotel:
                            </p>
                    ...
                    <template v-else>
                        <p class="mt-3 text-sm text-slate-500">El hotel confirmará tu grupo directamente. Guarda tu folio.</p>
                        <p v-if="holdCountdown" class="mt-2 text-xs text-warning">Se libera solo en {{ holdCountdown }} si el hotel no confirma antes.</p>
                    </template>
                </div>

                <!-- ═══ Datos del responsable ═══ -->
                <div v-else-if="step === 'guest'" class="p-5 sm:p-7">
                    <button type="button" class="mb-4 flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700" @click="step = 'search'">
                        <Lucide icon="ArrowLeft" class="h-4 w-4" /> Ajustar habitaciones
                    </button>

                    <div class="rounded-xl bg-slate-50 p-4 sm:p-5">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <div class="text-sm font-medium text-slate-800">Tu grupo</div>
                            ...
                            >
                                <div class="min-w-0">
                                    <div class="truncate text-sm text-slate-700">
                                        <span class="font-medium text-slate-800">{{ lines[option.room_type_id].rooms }}×</span>
                                        {{ option.name }}
                                    </div>
                                    <div class="mt-0.5 text-xs text-slate-400">{{ occupancyLabel(lines[option.room_type_id]) }}</div>
                                </div>
                                <div class="shrink-0 text-sm text-slate-600">
                                    {{ money(lines[option.room_type_id].rooms * option.total) }}
                                </div>
                            </div>
                        </div>
                        <div v-if="experienceLines.length" class="mt-2 divide-y divide-slate-200/80 border-t border-slate-200">
                        ...
                        <div class="flex items-center justify-between border-t border-slate-200 pt-3">
                            ...
                            <span class="text-base font-semibold text-slate-800">{{ money(estimatedTotal) }}</span>
                        </div>
                        <p class="mt-2 text-[11px] text-slate-400">
                            El total final lo calcula el sistema al apartar (incluye cargos por persona extra si aplican).
                        </p>
                    </div>

                    <!-- Experiencias en las fechas del grupo: un solo cobro con todo -->
                    <div v-if="experiencesCatalog?.enabled" class="mt-5">
                        <h2 class="text-base font-medium text-slate-800">¿Agregan un recorrido?</h2>
                        <p class="mt-1 text-sm text-slate-500">Opcional — se suma al mismo apartado y al mismo pago del grupo.</p>
                        <div class="mt-3 space-y-3">
                            <div v-for="exp in experiencesCatalog.experiences" :key="exp.id" class="rounded-lg border border-slate-200/70 p-3">
                    ...
                    <h2 class="mt-5 text-base font-medium text-slate-800">Responsable del grupo</h2>
                    <div class="mt-3 space-y-4">
                        <div>
                            <FormLabel>Nombre completo *</FormLabel>
                            <FormInput v-model="guestName" type="text" placeholder="Quien responde por el grupo" />
                        </div>
                        ...
                        <div>
                            ...
                            <FormTextarea v-model="notes" rows="2" placeholder="Boda, evento, hora de llegada del grupo…" />
                        </div>
                    ...
                    >
                        ...
                        {{ submitting ? 'Apartando…' : `Apartar ${totalRooms} habitaciones` }}
                    </Button>
                    <p class="mt-2.5 text-center text-[11px] text-slate-400">
                        Todo o nada: si alguna habitación ya no está disponible, no se aparta ninguna. Se aparta por {{ holdMinutes }} minutos mientras confirmas.
                    </p>
                </div>

                <!-- ═══ Fechas + habitaciones ═══ -->
                <div v-else class="p-5 sm:p-7">
                    <h1 class="text-lg font-medium text-slate-800">¿Vienen en grupo? Arma tu reserva completa</h1>
                    <p class="mt-1 text-sm text-slate-500">Elige cuántas habitaciones de cada tipo — un solo folio y un solo pago.</p>

                    <div v-if="bothModesAvailable" class="mt-5 inline-flex rounded-xl bg-slate-100 p-1 text-sm">
                        <button type="button" class="rounded-lg px-4 py-2 font-medium transition" :class="mode === 'block' ? 'bg-white text-theme-1 shadow-sm' : 'text-slate-500'" @click="mode = 'block'">
                            {{ property.block_mode_label }}
                        </button>
                        <button type="button" class="rounded-lg px-4 py-2 font-medium transition" :class="mode === 'night' ? 'bg-white text-theme-1 shadow-sm' : 'text-slate-500'" @click="mode = 'night'">
                            Por noche(s)
                        </button>
                    </div>
                    ...
                    <Button variant="primary" class="mt-5 w-full shadow-md shadow-primary/20" :disabled="searching" @click="search">
                    ...
                    <div v-if="searched" class="mt-6 space-y-3">
                        <div
                            ...
                            :class="option.available ? 'border-slate-200' : 'border-slate-100 opacity-60'"
                        >
                            <div class="flex flex-wrap items-start gap-4 p-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="mt-0.5 text-xs text-slate-500">
                                        {{ money(option.total) }} por habitación · hasta {{ option.effective_capacity }} personas ·
                                        {{ option.available ? `quedan ${option.rooms_count}` : 'sin disponibilidad' }}
                                    </div>
                                    <p v-if="option.advance_error" class="mt-1 text-[11px] text-warning">{{ option.advance_error }}</p>

                                    <div v-if="option.available" class="mt-3 flex flex-wrap items-end gap-5">
                                        <div>
                                            <div class="mb-1 text-[11px] font-medium uppercase tracking-wide text-slate-400">Habitaciones</div>
                                            <div class="flex items-center gap-2">
                                                <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 disabled:opacity-30" :disabled="!lines[option.room_type_id]?.rooms" @click="setRooms(option, (lines[option.room_type_id]?.rooms ?? 0) - 1)">
                                                    <Lucide icon="Minus" class="h-3.5 w-3.5" />
                                                </button>
                                                <span class="w-5 text-center text-sm font-medium">{{ lines[option.room_type_id]?.rooms ?? 0 }}</span>
                                                <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 disabled:opacity-30" :disabled="(lines[option.room_type_id]?.rooms ?? 0) >= Math.min(option.rooms_count, 15)" @click="setRooms(option, (lines[option.room_type_id]?.rooms ?? 0) + 1)">
                                                    <Lucide icon="Plus" class="h-3.5 w-3.5" />
                                                </button>
                                            </div>
                                        </div>
                                        <template v-if="(lines[option.room_type_id]?.rooms ?? 0) > 0">
                                            <div>
                                                <div class="mb-1 text-[11px] font-medium uppercase tracking-wide text-slate-400">Adultos c/u</div>
                                                <div class="flex items-center gap-2">
                                                    <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 disabled:opacity-30" :disabled="(lines[option.room_type_id]?.adults ?? 1) <= 1" @click="setAdults(option, lines[option.room_type_id].adults - 1)">
                                                        <Lucide icon="Minus" class="h-3.5 w-3.5" />
                                                    </button>
                                                    <span class="w-5 text-center text-sm font-medium">{{ lines[option.room_type_id]?.adults ?? 2 }}</span>
                                                    <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 disabled:opacity-30" :disabled="(lines[option.room_type_id]?.adults ?? 0) + (lines[option.room_type_id]?.children ?? 0) >= option.effective_capacity" @click="setAdults(option, lines[option.room_type_id].adults + 1)">
                                                        <Lucide icon="Plus" class="h-3.5 w-3.5" />
                                                    </button>
                                                </div>
                                            </div>
                                            <div v-if="!adultsOnly">
                                                <div class="mb-1 text-[11px] font-medium uppercase tracking-wide text-slate-400">Niños c/u</div>
                                                <div class="flex items-center gap-2">
                                                    <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 disabled:opacity-30" :disabled="!(lines[option.room_type_id]?.children ?? 0)" @click="setChildren(option, lines[option.room_type_id].children - 1)">
                                                        <Lucide icon="Minus" class="h-3.5 w-3.5" />
                                                    </button>
                                                    <span class="w-5 text-center text-sm font-medium">{{ lines[option.room_type_id]?.children ?? 0 }}</span>
                                                    <button type="button" class="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 disabled:opacity-30" :disabled="(lines[option.room_type_id]?.adults ?? 0) + (lines[option.room_type_id]?.children ?? 0) >= option.effective_capacity" @click="setChildren(option, lines[option.room_type_id].children + 1)">
                                                        <Lucide icon="Plus" class="h-3.5 w-3.5" />
                                                    </button>
                                                </div>
                                            </div>
                        ...
                        <div v-if="totalRooms > 0" class="rounded-xl bg-slate-50 px-4 py-3">
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-slate-500">{{ totalRooms }} habitaciones · {{ totalPeople }} personas</span>
                                <span class="font-semibold text-slate-800">{{ money(estimatedTotal) }}</span>
                            </div>
                            <p v-if="totalRooms === 1" class="mt-1 text-[11px] text-warning">
                                Un grupo son dos habitaciones o más — para una sola usa la
                                <a href="/reservar" class="font-medium underline">reserva normal</a>.
                            </p>
                        </div>

                        <Button variant="primary" class="w-full shadow-md shadow-primary/20" :disabled="totalRooms < 2" @click="continueToGuest">
                            <Lucide icon="ArrowRight" class="mr-2 h-4 w-4" /> Continuar con {{ totalRooms }} habitaciones
                        </Button>
                    </div>
                </div>
            </div>
            ...
            <p class="mt-4 text-center text-xs text-white/80">
                ...
                <a href="/reservar" class="font-medium text-white underline-offset-2 hover:underline">Reserva individual aquí</a>
            </p>
            <p class="mt-2 text-center text-[11px] text-white/60">Impulsado por KuiraWebReserve · tus datos de pago nunca pasan por este sitio</p>
        </div>
    </div>
</template>
