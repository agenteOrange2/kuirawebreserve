...
async function load() {
    ...
    try {
        const response = await axios.get<RackData>('/api/reservations/rack', {
            params: {
                from: localDateString(gridStart.value),
                days: weekCount.value * 7,
            },
        });
    ...
    for (const group of data.value.groups) {
        for (const room of group.rooms) {
            for (const entry of room.entries) {
                ...
                const last = new Date(
                    Math.max(
                        new Date(`${entry.end}T00:00:00`).getTime() - DAY_MS,
                        start.getTime(),
                    ),
                );
                for (
...
<template>
    <div class="box box--stacked overflow-hidden">
        ...
        <template v-else-if="data">
            <!-- Días de la semana -->
            <div
                class="grid grid-cols-7 border-b border-slate-200/60 dark:border-darkmode-400"
            >
                <div
                    v-for="label in weekdayLabels"
                    :key="label"
                    class="px-2 py-2 text-center text-xs font-medium tracking-wide text-slate-400 uppercase"
                >
                    {{ label }}
                </div>
            </div>
            ...
            <!-- Cuadrícula del mes -->
            <div
                class="grid grid-cols-7 gap-px bg-slate-200/60 dark:bg-darkmode-400"
            >
                <div
                    v-for="cell in cells"
                    :key="cell.day"
                    ...
                    :title="
                        canManage
                            ? `Crear reserva llegando el ${cell.day}`
                            : undefined
                    "
                ...
                >
                    ...
                    <button
                        ...
                        :class="[
                            event.isStart
                                ? toneSolid[event.entry.tone]
                                : toneSoft[event.entry.tone],
                            event.entry.reservation_id
                        ...
                        @click.stop="
                            event.entry.reservation_id &&
                            emit('open-reservation', event.entry.reservation_id)
                        "
                    >
        ...
        >
            Haz clic en una reserva para abrir su detalle; en un día vacío, para
            crear una reserva llegando esa fecha. El chip sólido marca el día de
            llegada; el suave, la continuación de la estancia.
        </div>
    </div>
</template>
