...
const emit = defineEmits<{
    ...
    (e: 'create', payload: { room: { id: number; number: string; room_type: string | null; rate_plan_id: number | null }; date: string }): void;
}>();
...
const dayFormatter = new Intl.DateTimeFormat('es-MX', { weekday: 'short', day: 'numeric' });
const dayLabel = (date: string) => dayFormatter.format(new Date(`${date}T00:00:00`));
...
const dayCellBg = (day: string) =>
    day === data.value?.today ? 'bg-primary/5' : isWeekend(day) ? 'bg-slate-50/70 dark:bg-darkmode-700/40' : '';

const dayIndex = (date: string) =>
    Math.round((Date.parse(`${date}T00:00:00`) - Date.parse(`${data.value?.from}T00:00:00`)) / DAY_MS);
...
function barsFor(room: RackRoom) {
    ...
    return room.entries
        .map((entry) => {
            ...
            const endIdx = Math.min(total, Math.max(dayIndex(entry.end), dayIndex(entry.start) + 1));
            return { entry, startIdx, endIdx };
        })
        .filter((bar) => bar.endIdx > 0 && bar.startIdx < total && bar.endIdx > bar.startIdx);
}
...
const totalRooms = computed(() => data.value?.groups.reduce((sum, group) => sum + group.rooms.length, 0) ?? 0);
</script>
    ...
    <div class="box box--stacked overflow-hidden">
        ...
        <div class="flex flex-wrap items-center gap-3 border-b border-slate-200/60 px-5 py-4 dark:border-darkmode-400">
            <div class="flex items-center gap-1.5">
                <Button variant="outline-secondary" class="!px-2.5" title="Semana anterior" @click="shiftDays(-7)">
                    <Lucide icon="ChevronLeft" class="h-4 w-4" />
                </Button>
                <Button variant="outline-secondary" @click="goToday">Hoy</Button>
                <Button variant="outline-secondary" class="!px-2.5" title="Semana siguiente" @click="shiftDays(7)">
        ...
        <div v-else-if="data && !totalRooms" class="px-5 py-10 text-center text-sm text-slate-500">
            Sin habitaciones aún: el calendario se llena solo cuando des de alta habitaciones.
        </div>

        <div v-else-if="data" class="overflow-x-auto">
            <div :style="{ minWidth: `${160 + (data.days.length ?? 0) * 56}px` }">
                <!-- Encabezado de días -->
                <div class="flex border-b border-slate-200/60 dark:border-darkmode-400">
                    <div
                        class="sticky left-0 z-20 w-40 shrink-0 border-r border-slate-200/60 bg-white px-4 py-2.5 text-xs font-medium uppercase tracking-wide text-slate-400 dark:border-darkmode-400 dark:bg-darkmode-600"
                    >
                        Habitación
                    </div>
                    <div class="grid flex-1" :style="gridStyle">
                        <div
                            ...
                            :class="[dayCellBg(day), day === data.today ? 'font-semibold text-primary' : 'text-slate-500']"
                        >
                            {{ dayLabel(day) }}
                        </div>
                    </div>
                ...
                <template v-for="group in data.groups" :key="group.type_id ?? group.type">
                    ...
                    <div class="flex border-b border-slate-200/60 dark:border-darkmode-400">
                        ...
                        <div class="grid flex-1 bg-slate-50 dark:bg-darkmode-700/60" :style="gridStyle">
                            <div
                                v-for="day in data.days"
                                :key="day"
                    ...
                    >
                        ...
                        >
                            <span v-if="room.zone_color" class="h-2 w-2 shrink-0 rounded-full" :style="{ backgroundColor: room.zone_color }" />
                            {{ room.number }}
                            <span v-if="room.zone" class="truncate text-xs font-normal text-slate-400">{{ room.zone }}</span>
                        </div>
                        <div class="relative grid flex-1" :style="gridStyle">
                            ...
                            <button
                                ...
                                :class="[
                                    dayCellBg(day),
                                    canManage ? 'cursor-pointer hover:bg-success/10' : 'cursor-default',
                                ]"
                                :style="{ gridColumn: `${i + 1} / ${i + 2}`, gridRow: '1' }"
                                :title="canManage ? `Reservar la ${room.number} llegando el ${day}` : undefined"
                                @click="
                                    canManage &&
                                    emit('create', {
                                        room: { id: room.id, number: room.number, room_type: group.type, rate_plan_id: group.rate_plan_id },
                                        date: day,
                                    })
                                "
                            />
                            ...
                            <div
                                ...
                                :class="[toneBg[bar.entry.tone], bar.entry.reservation_id ? 'cursor-pointer hover:brightness-110' : 'cursor-default']"
                                :style="{ gridColumn: `${bar.startIdx + 1} / ${bar.endIdx + 1}`, gridRow: '1' }"
                                :title="`${bar.entry.guest} · ${bar.entry.status_label} · ${bar.entry.time_range}${bar.entry.code ? ` · ${bar.entry.code}` : ''}`"
                                @click.stop="bar.entry.reservation_id && emit('open-reservation', bar.entry.reservation_id)"
                            >
                                <span class="truncate">{{ bar.entry.guest }}</span>
                            </div>
                        </div>
                    </div>
                </template>
        ...
        <div class="border-t border-slate-200/60 px-5 py-3 text-xs text-slate-400 dark:border-darkmode-400">
            Haz clic en una barra para ver el detalle de la reserva; en un hueco, para crear una reserva con esa habitación y fecha.
            Los walk-ins en casa se muestran pero su detalle vive en la sección "En casa ahora" de la lista de reservas.
        </div>
    </div>
</template>
