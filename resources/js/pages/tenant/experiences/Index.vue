    ...
    try {
        if (editing.value) {
            ...
            toast.success(
                ...
                'Las reservas ya hechas conservan su total congelado.',
            );
        } else {
            await axios.post('/api/experiences', payload);
            toast.success(
                'Experiencia creada',
                'Agrega sesiones con cupo para que se pueda reservar.',
            );
        }
...
async function destroy() {
    ...
    try {
        ...
        toast.success(
            ...
            'Sus sesiones y reservas se eliminaron con ella.',
        );
        deleting.value = null;
        reload();
    } catch {
...
async function uploadPhotos(event: Event) {
    ...
    try {
        const { data } = await axios.post<{ photos: Photo[] }>(
            `/api/experiences/${editing.value.id}/photos`,
            formData,
        );
        ...
        toast.error(
            ...
            e.response?.data?.message ??
                (errs
                    ...
                    : 'Revisa formato y peso (máx. 6 MB).'),
        );
    } finally {
...
async function removePhoto(photo: Photo) {
    ...
    photoBusy.value = true;
    try {
        const { data } = await axios.delete<{ photos: Photo[] }>(
            `/api/experiences/${editing.value.id}/photos/${photo.id}`,
        );
        photos.value = data.photos;
        reload();
    } catch {
...
async function makeCover(photo: Photo) {
    ...
    try {
        const order = [
            photo.id,
            ...photos.value.filter((p) => p.id !== photo.id).map((p) => p.id),
        ];
        const { data } = await axios.patch<{ photos: Photo[] }>(
            `/api/experiences/${editing.value.id}/photos/order`,
            { order },
        );
        photos.value = data.photos;
        reload();
...
async function addVehicle() {
    ...
    try {
        const { data } = await axios.post<VehicleRow>(
            '/api/experience-vehicles',
            {
        ...
        toast.success(
            'Vehículo agregado',
            'Asígnalo a los horarios de tus experiencias.',
        );
        reload();
    } catch (e: any) {
        toast.error(
            'No se pudo agregar',
            e.response?.data?.message ?? 'Revisa nombre y capacidad.',
        );
    } finally {
...
async function toggleVehicle(vehicle: VehicleRow) {
    try {
        const { data } = await axios.patch<VehicleRow>(
            `/api/experience-vehicles/${vehicle.id}`,
        ...
        toast.error(
            'Error',
            e.response?.data?.message ?? 'No se pudo actualizar el vehículo.',
        );
    }
}
...
async function deleteVehicle(vehicle: VehicleRow) {
    try {
        ...
        toast.success(
            'Vehículo eliminado',
            'Los horarios que lo usaban ajustaron su cupo.',
        );
        reload();
    } catch (e: any) {
        toast.error(
            'No se pudo eliminar',
            e.response?.data?.message ?? 'Intenta desactivarlo en su lugar.',
        );
    }
}
...
function scheduleSummary(experience: ExperienceRow): string {
    ...
    if (!experience.operating_days.length || !liveSlots)
        return 'Sin programación semanal';
    const days = WEEKDAYS.filter((d) =>
        experience.operating_days.includes(d.value),
    )
        .map((d) => d.label)
        .join(', ');
...
async function toggleDay(day: number) {
    ...
    try {
        await axios.patch(`/api/experiences/${scheduleFor.value.id}`, {
            operating_days: next,
        });
        ...
        toast.error(
            'Error',
            e.response?.data?.message ?? 'No se pudieron guardar los días.',
        );
    } finally {
...
async function addSlot() {
    ...
    try {
        const { data } = await axios.post<SlotRow>(
            `/api/experiences/${scheduleFor.value.id}/slots`,
            {
                start_time: slotForm.start_time,
                vehicle_ids: slotForm.vehicle_ids,
                capacity:
                    slotForm.capacity === '' ? null : Number(slotForm.capacity),
            },
        );
        scheduleFor.value.slots = [...scheduleFor.value.slots, data].sort(
        ...
        toast.success(
            'Horario agregado',
            'Las sesiones del horizonte de venta ya se generaron.',
        );
        reload();
    } catch (e: any) {
        const data = e.response?.data;
...
async function toggleSlot(slot: SlotRow) {
    if (!scheduleFor.value) return;
    try {
        const { data } = await axios.patch<SlotRow>(
            `/api/experiences/${scheduleFor.value.id}/slots/${slot.id}`,
            { active: !slot.active },
        );
        scheduleFor.value.slots = scheduleFor.value.slots.map((s) =>
            s.id === data.id ? data : s,
        );
        reload();
    } catch (e: any) {
        toast.error(
            'Error',
            e.response?.data?.message ?? 'No se pudo actualizar el horario.',
        );
    }
}
...
async function deleteSlot(slot: SlotRow) {
    ...
    try {
        ...
        scheduleFor.value.slots = scheduleFor.value.slots.filter(
            (s) => s.id !== slot.id,
        );
        toast.success(
            'Horario eliminado',
            'Sus sesiones futuras sin reservas se quitaron de la venta.',
        );
        reload();
    } catch (e: any) {
        toast.error(
            'Error',
            e.response?.data?.message ?? 'No se pudo eliminar el horario.',
        );
    }
}
...
function vehicleNames(slot: SlotRow): string {
    const names = vehicles.value
        .filter((v) => slot.vehicle_ids.includes(v.id))
        .map((v) => v.name);
...
async function addSession() {
    ...
    Object.keys(sessionErrors).forEach((k) => delete sessionErrors[k]);
    try {
        const { data } = await axios.post<SessionRow>(
            `/api/experiences/${sessionsFor.value.id}/sessions`,
            {
                starts_at: sessionForm.starts_at,
                capacity: sessionForm.capacity,
            },
        );
        sessionsFor.value.sessions = [...sessionsFor.value.sessions, data].sort(
            (a, b) => a.starts_at.localeCompare(b.starts_at),
        );
        sessionForm.starts_at = '';
        toast.success(
            'Sesión creada',
            'Ya se puede reservar en la página pública.',
        );
        reload();
    } catch (e: any) {
        const data = e.response?.data;
        if (data?.errors) {
            Object.entries(data.errors).forEach(
                ([key, msgs]) => (sessionErrors[key] = (msgs as string[])[0]),
            );
        } else {
            toast.error(
                'Error',
...
async function cancelSession(session: SessionRow) {
    ...
    try {
        const { data } = await axios.patch(
            `/api/experiences/${sessionsFor.value.id}/sessions/${session.id}`,
            { status: 'cancelled' },
        );
        ...
        toast.success(
            ...
            data.cancelled_bookings
                ? `Se cancelaron ${data.cancelled_bookings} reserva(s); avisa a los inscritos.`
                : undefined,
        );
        reload();
    } catch (e: any) {
...
async function deleteSession(session: SessionRow) {
    ...
    try {
        ...
        sessionsFor.value.sessions = sessionsFor.value.sessions.filter(
            (s) => s.id !== session.id,
        );
        reload();
    } catch (e: any) {
        toast.error(
            'No se pudo borrar',
            e.response?.data?.message ?? 'Intenta cancelarla en su lugar.',
        );
    }
}
...
const bookableSessions = computed(() => {
    const experience = props.experiences.find(
        (e) => e.id === Number(bookingForm.experience_id),
    );
    return (experience?.sessions ?? []).filter(
        (s) => s.status === 'scheduled' && s.remaining > 0,
    );
});
...
async function submitBooking() {
    ...
    Object.keys(bookingErrors).forEach((k) => delete bookingErrors[k]);
    try {
        const { data } = await axios.post<BookingRow>(
        ...
        toast.success(
            'Reserva registrada',
            `Folio ${data.code} · total ${money(data.total)}.`,
        );
        reload();
    } catch (e: any) {
        const data = e.response?.data;
        if (data?.errors) {
            Object.entries(data.errors).forEach(
                ([key, msgs]) => (bookingErrors[key] = (msgs as string[])[0]),
            );
        } else {
            toast.error(
                'No se pudo registrar',
                data?.message ?? 'Revisa el cupo de la sesión.',
            );
        }
    } finally {
        ...
        if (data.payment.checkout_url) {
            try {
                ...
                toast.success(
                    ...
                    `${data.payment.amount_label} — pégalo en el chat con el huésped.`,
                );
            } catch {
                toast.success(
                    'Cobro generado',
                    `${data.payment.amount_label} — copia el link desde la fila.`,
                );
            }
        } else {
            toast.success(
                'Cobro por transferencia emitido',
                `${data.payment.amount_label} — verifícalo en la cola de pagos cuando llegue el comprobante.`,
            );
        }
    } catch (e: any) {
        toast.error(
            ...
            e.response?.data?.message ??
                'Revisa las pasarelas o cuentas en Métodos de pago.',
        );
    } finally {
...
async function copyCheckout(booking: BookingRow) {
    ...
    try {
        await navigator.clipboard.writeText(
            booking.pending_payment.checkout_url,
        );
...
async function setBookingStatus(booking: BookingRow, status: string) {
    try {
        const { data } = await axios.patch<BookingRow>(
            `/api/experience-bookings/${booking.id}/status`,
            { status },
        );
        bookings.value = bookings.value.map((b) =>
            b.id === data.id ? data : b,
        ...
        toast.error(
            'Error',
            e.response?.data?.message ?? 'No se pudo actualizar la reserva.',
        );
    }
}
    ...
    <RazeLayout title="Experiencias">
        <div class="mt-2">
            ...
            <div v-if="experiences.length" class="mt-5 grid grid-cols-12 gap-5">
                ...
                >
                    <div
                        class="box box--stacked flex h-full flex-col overflow-hidden"
                    >
                        <div
                            class="relative h-40 w-full bg-slate-100 dark:bg-darkmode-400"
                        >
                            <img
                            ...
                            <span
                                ...
                                class="absolute top-2 left-2 rounded-full bg-slate-800/70 px-2 py-0.5 text-[10px] font-medium text-white"
                            >
                                Pausada
                            </span>
                        </div>
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="mt-0.5 text-xs text-slate-500">
                                        {{ experience.price_label
                                            ...
                                            v-if="experience.duration_label"
                                        >
                                            ·
                                            {{
            ...
            >
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-xs text-slate-500">
                        Crea la primera — un tour, un recorrido, una actividad —
                        y agrégale sesiones con cupo.
                    </p>
                </div>
                <Button
                    v-if="canManage"
            ...
            <div class="box box--stacked mt-6">
                <div
                    class="flex flex-wrap items-center justify-between gap-3 border-b border-dashed border-slate-300/70 px-5 py-4 dark:border-darkmode-400"
                >
                    <div>
                        <div
                            class="flex items-center gap-2 text-xs font-medium tracking-wide text-slate-400 uppercase"
                        >
                            <Lucide icon="TicketCheck" class="h-3.5 w-3.5" />
                            Reservas de experiencias
                        </div>
                        <p class="mt-1 text-xs text-slate-500">
                            Las pendientes van primero: confírmalas cuando el
                        ...
                        <Table.Tbody>
                            ...
                            >
                                ...
                                <Table.Td>
                                    ...
                                    >
                                        <Lucide icon="Copy" class="h-3 w-3" />
                                        Cobro vigente
                                    </button>
                                    <span
                                        v-else-if="booking.pending_payment"
                                        class="mt-0.5 inline-flex items-center gap-1 rounded-full bg-warning/10 px-2 py-0.5 text-[10px] font-medium text-warning"
                                    >
                ...
                >
                    Sin reservas próximas. Comparte la página pública o registra
                    una desde aquí.
                </div>
            </div>
        </div>
                ...
                <form class="flex flex-col" @submit.prevent="submit">
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 text-xs text-slate-500">
                                La ficha que ve el huésped en la página pública
                                de experiencias.
                            </p>
                        </div>
                        <button
                    ...
                    >
                            ...
                            <div class="grid grid-cols-12 gap-5">
                                <div class="col-span-12">
                                    ...
                                    <FormInput
                                        v-model="form.name"
                                        type="text"
                                        placeholder="Recorrido en cuatrimoto"
                                    />
                                    <FormHelp
                                        v-if="errors.name"
                                        class="text-danger"
                                ...
                                <div class="col-span-12">
                                    ...
                                    <div class="flex gap-2">
                                        <FormInput
                                            ...
                                            @keyup.enter.prevent="addInclude"
                                        />
                                        <Button
                                            type="button"
                                            variant="outline-secondary"
                                    ...
                                    >
                                        <span
                                            ...
                                            :key="item"
                                            class="flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600 dark:bg-darkmode-400 dark:text-slate-300"
                                        >
                                            {{ item }}
                                            <button
                                                type="button"
                            ...
                            <div class="grid grid-cols-12 gap-5">
                                <div class="col-span-12 sm:col-span-6">
                                    ...
                                    <FormSelect v-model="form.pricing_mode">
                                        <option
                                            v-for="(label, key) in pricingModes"
                                            :key="key"
                                            :value="key"
                                        >
                                            {{ label }}
                                    ...
                                    <FormHelp
                                        >"Por grupo": un precio fijo sin
                                        importar cuántos van (ej. la
                                        cuatrimoto).</FormHelp
                                    >
                                </div>
                                <div class="col-span-12 sm:col-span-6">
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    >
                                    <FormInput
                                        v-model.number="form.min_people"
                                        type="number"
                                        min="1"
                                    />
                                </div>
                        ...
                        >
                            ...
                            >
                                <Lucide icon="Image" class="h-3.5 w-3.5" />
                                Fotos
                            </div>
                            <p class="mb-4 text-xs text-slate-500">
                                La primera es la portada en la página pública.
                            </p>
                            <div
                                v-if="!editing"
                                class="rounded-lg border border-dashed border-slate-300/70 px-4 py-5 text-center text-xs text-slate-500 dark:border-darkmode-400"
                            >
                                Guarda la experiencia primero; después podrás
                                subir sus fotos desde aquí.
                            </div>
                            <template v-else>
                                <div
                                    v-if="photos.length"
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <span
                                            v-if="index === 0"
                                            class="absolute top-1.5 left-1.5 rounded-full bg-primary px-2 py-0.5 text-[10px] font-medium text-white"
                                            >Portada</span
                                        >
                                        <div
                                            class="absolute inset-x-0 bottom-0 flex justify-end gap-1 bg-gradient-to-t from-black/60 to-transparent p-1.5 opacity-0 transition group-hover:opacity-100"
                                        >
                                            <button
                                                v-if="index !== 0"
                                                type="button"
                                ...
                                <div
                                    ...
                                    class="rounded-lg border border-dashed border-slate-300/70 px-4 py-5 text-center text-xs text-slate-500 dark:border-darkmode-400"
                                >
                                    Sin fotos, la experiencia se muestra solo
                                    con texto.
                                </div>
                        ...
                        >
                            ...
                            >
                                <div class="text-sm">
                                    ...
                                    <p class="mt-0.5 text-xs text-slate-500">
                                        Pausada, no aparece ni se puede
                                        reservar; sus reservas existentes no
                                        cambian.
                                    </p>
                                </div>
                                <FormSwitch>
            ...
            <Dialog.Panel>
                <div class="p-5">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-base font-medium">
                                Sesiones de {{ sessionsFor?.name }}
                            </h2>
                            <p class="text-xs text-slate-500">
                                Cada sesión tiene su propio cupo; el cupo es
                                duro, nunca se sobrevende.
                            </p>
                        </div>
                    </div>

                    <p
                        v-if="sessionsFor?.sessions.length"
                        class="mb-2 text-xs text-slate-400"
                    >
                        Se muestran los próximos 60 días; la venta está abierta
                        todo el año.
                    </p>
                    <div
                        v-if="sessionsFor?.sessions.length"
                    ...
                    >
                        ...
                        >
                            <div class="min-w-0">
                                ...
                                <div class="text-xs text-slate-500">
                                    {{ session.people_booked }} de
                                    {{ session.capacity }} lugares vendidos
                                    <span
                                        v-if="session.from_schedule"
                            ...
                            >
                                <button
                                    ...
                                    title="Cancelar sesión (cancela sus reservas vivas)"
                                    @click="cancelSession(session)"
                                >
                                    <Lucide icon="Ban" class="h-4 w-4" />
                                </button>
                                <button
                                    v-if="session.people_booked === 0"
                                    type="button"
                                    class="rounded p-1.5 text-slate-400 transition hover:bg-danger/10 hover:text-danger"
                                    title="Borrar sesión"
                    ...
                    <div
                        ...
                        class="rounded-lg border border-dashed border-slate-300/70 px-4 py-5 text-center text-xs text-slate-500 dark:border-darkmode-400"
                    >
                        Sin sesiones: la experiencia no aparece en la página
                        pública hasta que agregues una con fecha futura.
                    </div>
                        ...
                        <div class="grid grid-cols-12 items-end gap-3">
                            <div class="col-span-12 sm:col-span-6">
                                ...
                                <FormHelp
                                    ...
                                    class="text-danger"
                                    >{{ sessionErrors.starts_at }}</FormHelp
                                >
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label class="mb-1 block text-sm">Cupo</label>
                                <FormInput
                                    v-model.number="sessionForm.capacity"
                                    type="number"
                                    min="1"
                                    max="500"
                                />
                                <FormHelp
                                    v-if="sessionErrors.capacity"
                                    class="text-danger"
                                    >{{ sessionErrors.capacity }}</FormHelp
                                >
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <Button
                                    ...
                                    :disabled="
                                        sessionSaving || !sessionForm.starts_at
                                    "
                                    @click="addSession"
                                >
            ...
            <Dialog.Panel>
                <div class="p-5">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-base font-medium">
                                Vehículos para experiencias
                            </h2>
                            <p class="text-xs text-slate-500">
                                La flota es del hotel: el mismo vehículo puede
                                servir a varias experiencias en horarios
                                distintos.
                            </p>
                        </div>
                    </div>
                        ...
                        >
                            ...
                            >
                                <button
                                    ...
                                    :title="
                                        vehicle.active
                                            ? 'Sacar de servicio (los horarios que lo usan bajan su cupo)'
                                            : 'Regresar a servicio'
                                    "
                    ...
                    <div
                        ...
                        class="rounded-lg border border-dashed border-slate-300/70 px-4 py-5 text-center text-xs text-slate-500 dark:border-darkmode-400"
                    >
                        Sin vehículos: agrega el primero (razer, camioneta,
                        cuatrimoto...) con cuántas personas lleva.
                    </div>
                        ...
                        <div class="grid grid-cols-12 items-end gap-3">
                            <div class="col-span-12 sm:col-span-6">
                                ...
                                <FormInput
                                    ...
                                    type="text"
                                    placeholder="Razer 1, camioneta roja…"
                                />
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label class="mb-1 block text-sm"
            ...
            <Dialog.Panel>
                <div class="p-5">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-base font-medium">
                                Programación de {{ scheduleFor?.name }}
                            </h2>
                            <p class="text-xs text-slate-500">
                                Días que opera y horarios con sus vehículos. Las
                                sesiones de todo el año se generan y actualizan
                                solas — abierto a la venta como las
                                habitaciones.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="max-h-[70vh] overflow-y-auto">
                        <section>
                            ...
                            <div class="mt-2 flex flex-wrap gap-2">
                                <button
                                    ...
                                    :class="
                                        scheduleFor?.operating_days.includes(
                                            day.value,
                                        )
                            ...
                            <FormHelp
                                >Ejemplo: solo viernes a domingo = 3 días. Sin
                                días marcados no se genera ninguna
                                sesión.</FormHelp
                            >
                        </section>
                        ...
                        <section class="mt-5">
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            ...
                                            :title="
                                                slot.active
                                                    ? 'Pausar horario (poda sus sesiones futuras sin reservas)'
                                                    : 'Reactivar horario'
                                            "
                            ...
                            <div
                                v-else
                                class="mt-2 rounded-lg border border-dashed border-slate-300/70 px-4 py-5 text-center text-xs text-slate-500 dark:border-darkmode-400"
                            >
                                Sin horarios: agrega el primero (por ejemplo
                                10:00 con el razer y la camioneta).
                            </div>
                            ...
                            >
                                <div class="grid grid-cols-12 gap-3">
                                    ...
                                    <div class="col-span-12 sm:col-span-6">
                                        <label class="mb-1 block text-sm"
                                            >Vehículos de este horario</label
                                        >
                                        <div
                                            v-if="
                                        ...
                                        <FormHelp v-else
                                            >Sin vehículos activos: usa el cupo
                                            manual o agrega vehículos
                                            primero.</FormHelp
                                        >
                                    </div>
                                </div>
                                <FormHelp
                                    >Sin cupo manual, el cupo es la suma de los
                                    vehículos marcados.</FormHelp
                                >
            ...
            <Dialog.Panel>
                <form class="p-5" @submit.prevent="submitBooking">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-base font-medium">
                                Registrar reserva de experiencia
                            </h2>
                            <p class="text-xs text-slate-500">
                                Para huéspedes que llaman o llegan a recepción.
                                El total lo calcula el sistema.
                            </p>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <div>
                            ...
                            >
                            <FormSelect
                                v-model="bookingForm.experience_id"
                                @change="bookingForm.experience_session_id = ''"
                            >
                        ...
                        <div>
                            ...
                            <FormSelect
                                v-model="bookingForm.experience_session_id"
                                :disabled="!bookingForm.experience_id"
                            >
                                <option value="" disabled>
                                    Elige fecha y horario
                                </option>
                                <option
                                    v-for="session in bookableSessions"
                                    :key="session.id"
                                    :value="session.id"
                                >
                                    {{ formatDateTime(session.starts_at) }} ·
                                    {{ session.remaining }} lugar(es)
                                </option>
                            </FormSelect>
                            <FormHelp
                                v-if="
                                ...
                                "
                                >Esa experiencia no tiene sesiones con cupo;
                                crea una primero.</FormHelp
                            >
                        </div>
                        ...
                        <div>
                            ...
                            <FormInput
                                ...
                                type="text"
                                placeholder="Como se presenta"
                            />
                            <FormHelp
                                v-if="bookingErrors.guest_name"
                                class="text-danger"
                                >{{ bookingErrors.guest_name }}</FormHelp
                            >
                        </div>
                        <div>
                            <label class="mb-1 block text-sm"
                            ...
                            <FormInput
                                ...
                                type="text"
                                placeholder="Pagó en efectivo, llega 10 min antes…"
                            />
                        </div>
                        <div
                            class="flex items-center justify-between rounded-lg border border-dashed border-slate-300/70 px-3 py-2.5 dark:border-darkmode-400"
                        >
                            ...
                            <FormSwitch>
                                <FormSwitch.Input
                                    :checked="bookingForm.confirmed"
                                    type="checkbox"
                                    @change="
                                        bookingForm.confirmed =
                                            !bookingForm.confirmed
                                    "
                                />
                            </FormSwitch>
            ...
            <Dialog.Panel>
                <div class="p-5 text-center">
                    ...
                    <p class="mt-2 text-sm text-slate-500">
                        Se eliminan también sus sesiones y reservas. Si solo
                        quieres dejar de venderla, pausa la experiencia con el
                        switch del editor.
                    </p>
