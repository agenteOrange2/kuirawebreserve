...
function submitProduct() {
    const payload: Record<string, unknown> = {
        ...
        track_stock:
            productForm.type === 'simple' ? productForm.track_stock : false,
        reorder_point:
...
function submitDeleteProduct() {
    ...
    mutate(
        () => axios.delete(`/api/products/${deletingProduct.value!.id}`),
    ...
    mutate(
        () =>
            editingIngredient.value
                ? axios.patch(
                      `/api/ingredients/${editingIngredient.value.id}`,
                      payload,
                  )
...
function openMovement(
    kind: 'product' | 'ingredient',
    row: { id: number; name: string; unit: string; stock_qty: number },
) {
...
function submitMovement() {
    ...
    const base =
        movementTarget.value.kind === 'product' ? 'products' : 'ingredients';
...
const tabs = computed(() => [
    ...
    },
    {
        key: 'movimientos',
        label: 'Movimientos',
        icon: 'ArrowRightLeft' as Icon,
        count: props.movements.length,
    },
]);
</script>
...
<template>
    <RazeLayout title="Inventario">
        <div class="mt-2">
            ...
            <div
                v-if="generalError"
                class="mt-4 rounded-lg bg-danger/10 px-4 py-3 text-sm text-danger"
            >
                {{ generalError }}
            </div>
                                    ...
                                    ><span
                                        :class="
                                            i.low_stock
                                                ...
                                                : ''
                                        "
                                        >{{ i.stock_qty }} {{ i.unit }}</span
                                    ></Table.Td
                                >
                                <Table.Td
                                    :class="cellClass"
                                    class="text-right text-slate-500"
                                    >{{ i.reorder_point ?? '—' }}</Table.Td
                                >
                                <Table.Td
                                    :class="cellClass"
                                    class="text-right text-slate-500"
            ...
            <div v-show="tab === 'movimientos'" class="mt-4">
                ...
                >
                        ...
                        <ul class="-mb-4">
                            ...
                            >
                                <span
                                    ...
                                    class="absolute top-10 left-[19px] h-full w-px bg-slate-200 dark:bg-darkmode-400"
                                />
                                <div class="flex items-start gap-3">
                                    <div
                                        class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full"
                                        :class="
                                            movementMeta[m.type]?.tone ??
                                            'bg-slate-100 text-slate-500'
                                        "
                                    >
                                        <Lucide
                                            :icon="
                                                movementMeta[m.type]?.icon ??
                                                'Circle'
                                            "
                                            class="h-4 w-4"
                                        />
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        >
                                            ...
                                            >
                                            <span
                                                v-if="m.unit_cost !== null"
                                                class="text-xs text-slate-400"
                                                >@
                                                {{ money(m.unit_cost) }}</span
                                            >
                                        </div>
                    ...
                    >
                        ...
                        <p class="text-sm text-slate-500">
                            Sin movimientos aún. Compras, mermas, ajustes y
                ...
                >
                    ...
                    <div class="flex-1 space-y-4 overflow-y-auto px-6 py-5">
                        ...
                        <template v-if="productForm.type === 'composite'">
                            ...
                            >
                                ...
                                >
                                    <Lucide
                                        ...
                                        class="h-3.5 w-3.5"
                                    />
                                    Receta (se descuenta de insumos)
                                </div>
                                <Button
                                    type="button"
                                    variant="outline-primary"
                            ...
                            >
                                Primero crea insumos para armar la receta.
                            </p>
                            <div class="space-y-2">
                                <div
                                    v-for="(row, idx) in productForm.recipe"
                                    :key="idx"
                                ...
                                >
                                    ...
                                    <button
                                        type="button"
                                        class="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 transition hover:bg-danger/10 hover:text-danger"
                                        @click="
                                            productForm.recipe.splice(idx, 1)
                                        "
                                    >
                ...
                <form class="flex flex-col" @submit.prevent="submitIngredient">
                    ...
                    <div class="space-y-4 px-6 py-5">
                        ...
                        >
                            ...
                            <div>
                                <FormLabel htmlFor="i-cost"
                                    >Costo unitario</FormLabel
                                >
                                <div class="relative">
                                    <Lucide
                                        icon="DollarSign"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    /><FormInput
                                        id="i-reorder"
                                        v-model="ingredientForm.reorder_point"
                                        type="number"
                                        step="0.001"
                                        min="0"
                                        class="pl-9"
                ...
                >
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 text-xs text-slate-500">
                                {{ movementTarget.name }} ·
                                {{ movementTarget.stock_qty }}
                                {{ movementTarget.unit }} en existencia
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="space-y-4 px-6 py-5">
                        ...
                        <div
                            v-if="movementForm.qty !== ''"
                            class="flex items-center justify-between rounded-lg bg-slate-50 px-4 py-3 text-sm dark:bg-darkmode-700"
                        >
                            <span class="text-slate-500"
                                >Existencia resultante</span
                            >
                            <span
                                class="font-medium"
                                :class="
                                ...
                                >{{ movementTarget.stock_qty }} →
                                {{ resultingStock }}
                                {{ movementTarget.unit }}</span
                            >
                        </div>
                        <div>
                            <FormLabel htmlFor="m-notes"
                                >Notas (opcional)</FormLabel
                            >
                            <FormInput
                                id="m-notes"
