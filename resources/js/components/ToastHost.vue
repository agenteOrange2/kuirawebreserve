...
const iconFor = (toast: Toast) =>
    ({ success: 'CircleCheck', error: 'CircleAlert', info: 'Info' })[
        toast.type
    ] as 'CircleCheck' | 'CircleAlert' | 'Info';
...
<template>
    <div
        class="pointer-events-none fixed top-20 right-5 z-[70] flex w-[340px] max-w-[calc(100vw-2.5rem)] flex-col gap-2"
    >
        <TransitionGroup
        ...
        >
            ...
            >
                <Lucide
                    :icon="iconFor(toast)"
                    :class="['mt-0.5 h-5 w-5 shrink-0', toneFor(toast)]"
                />
